...
- **INSTRUMENT — le juge (dys_pipeline_probe) ne voit pas les accents : 273 fautes d'accent seul invisibles, 4 « a → à » cassés invisibles** — Vu le 11/09/2026 en posant _AFIX_MIN : 5 réparations réelles dans les 72 productions, chiffre de référence inchangé — `DP.eq` compare via `norm()` qui désaccentue. Comptage strict (accent_strict.py, scratchpad) : 273 fautes d'accent seul sur 6 217 mots, 160 → 165 réparées, et 4 accents cassés sur des mots justes (a → à ×4) que la colonne « cassés » ne voit pas. À faire AVANT tout chantier de moteur sur les accents : porter le comptage strict dans la sonde (colonnes « accents réparés » / « accents cassés »), ancrer, puis regarder les 4 a → à un par un (rouge prioritaire). _(JOURNAL 2026-09-11 (nuit, suite) · dictee/dys_precision_probe.py norm()/eq())_
