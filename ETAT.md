...
| 33 | accord de PALIER produit↔référence sur le gold (auto/flag/vigilance ; SAUTÉ sans corpus dys) | `python3 dictee/palier_gold_probe.py` |
...
- **Tokeniseur du speller Python ≠ toks du produit (apostrophes)** — Nommé le 06/09/2026 par la garde d'accord de palier : « d'une pérsone », « l'est conporte » — UN token côté produit (`toks`), deux côté référence (`TOK` sans apostrophe), donc le produit ne voit ni déterminant ni copule avant le mot et PROPOSE là où la référence AFFIRME. Les 2 sont ancrés avec leur cause dans palier_gold_ref.json. Jonction suivante de la série « la référence décrit le produit ». _(JOURNAL 2026-09-06 · palier_gold_ref.json)_
