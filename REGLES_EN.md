...
**Deux contrats du pipeline, trouvés en essayant la page en direct (18/09/2026).** ① Une marque peut couvrir DEUX mots : quand la suggestion est ce mot et le suivant collés (« with out » → without), `analyzeText` pose `span:2` et ne marque pas le second ; toute page qui applique une suggestion doit remplacer les deux (la page du correcteur et la page vocale écrivaient « without out »). Une seule règle concernée sur 28 049 textes, mais le test est général. ② L'ORDRE DE CHARGEMENT : quatre tables (`_P2P`, `_V3PAST`, deux tables de confusables) étaient bâties au premier appel puis gardées. Une page semi-directe appelle le moteur dès que le dictionnaire est là, avant `verbmorph_en.json` et `confusables_en.json` : les tables se bâtissaient sur du vide et « has went », « I will council him », « witch / which » restaient muets toute la session. Elles se rebâtissent maintenant quand leur source change ; l'auto-test du moteur rejoue l'ordre de la page (5 mutations tuées). **Toute table bâtie paresseusement sur un actif chargé à part doit garder la référence de sa source.** La page elle-même a sa garde : `correcteur_en_page_probe.js` (cf. CHANTIER_ANGLAIS.md, étape ①).
