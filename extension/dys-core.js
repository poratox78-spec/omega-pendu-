...
function estQuestion(t,maxMots){
  ...
  function _sujetFlexion(T,i,tg){   /* (personne, nombre) du sujet de T[i] — miroir Python _sujet_flexion. On remonte jusqu'à la
    ...
    while(j>=0){
      ...
      if(m)d=deacc(m[1]);
      if(SUBJ_PRON[d])return SUBJ_PRON[d];
      if(_SUBJ_PRON_PL[d]){
        var k=j-1;
        while(k>=0){
      ...
      if(tg&&j<tg.length&&(tg[j]==='NOUN'||tg[j]==='PROPN'))return null;   // sujet NOMINAL : laissé aux règles d'accord SV (mesuré : 12 FP UD)
  ...
  function sujFlexVig(T,i){   /* « nous iriez » → irions · « tu irai » → iras. Le sujet donne personne et nombre, la forme écrite
    ...
    if(CLITIC[dl]||PREP[dl]||MODAL[dl])return null;
    var lec=svReads(w);
