...
function estQuestion(t,maxMots){
  ...
  function _sujetFlexion(T,i,tg){   /* (personne, nombre) du sujet de T[i] — miroir Python _sujet_flexion. On remonte jusqu'à la
    ...
    while(j>=0){
      ...
      if(tg&&j<tg.length&&(tg[j]==='NOUN'||tg[j]==='PROPN')){var _np=_npSubject(T,tg,i);return (_np&&(_np.n==='s'||_np.n==='p'))?['3',_np.n,'nom']:null;}   // sujet NOMINAL → palier ORANGE (miroir Python)
