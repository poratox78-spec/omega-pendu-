...
var pn=svSubject(T,i);if(!pn)return null;var per=pn[0],nb=pn[1];
    ...
    if(per==='3'&&nb==='p'&&_dsv==='est'&&i+1<T.length&&!/s$/.test(deacc(T[i+1].toLowerCase()))){var _tgd=posTags(T);if(_isPpl(T[i+1])||(_tgd&&_tgd[i+1]==='VERB'&&/(é|ée)$/.test(T[i+1].toLowerCase())))return null;}   /* ⭐ 11/09/2026 (restes frgec, lot 2) : « Elles est fomée » — pronom PLURIEL + « est » + participe SINGULIER : verbe (audible) et participe s'accordent contre le -s muet du pronom → le pronom est suspect → abstention. Miroir Python. */
...
function estQuestion(t,maxMots){
  ...
  function sujFlexVig(T,i){   /* « nous iriez » → irions · « tu irai » → iras. Le sujet donne personne et nombre, la forme écrite
    ...
    if(src==='nom'&&_temoinApresVerbe(T,tg,i,nb))return null;   /* ⭐ 11/09/2026 (lot 2) : même évidence que rAccordSVnoun — l'orange se tait aussi (« Les impudents est le premier roman »). Miroir Python. */
