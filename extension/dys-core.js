...
(function (global) {
  ...
  function rOn(T,i){var lw=deacc(T[i].toLowerCase());if(lw!=='on'&&lw!=='ont')return null;var p=cprev(T,i);if(p==='ils'||p==='elles'||cpl(T,i-1))return 'ont';var nx=i+1<T.length?T[i+1].toLowerCase():'';if(/és?$/.test(nx)||/ées?$/.test(nx)||isParticiple(T,i+1)||IRREG_PART[deacc(nx)])return 'ont';if(vlike(T,i+1))return 'on';return null;}
  ...
  function rA(T,i){if(deacc(T[i].toLowerCase())!=='a')return null;if(T[i]===T[i].toUpperCase()&&T[i]!==T[i].toLowerCase())return null;var p=cprev(T,i);if(p==='il'||p==='elle'||p==='on'||p==='qui'||p==='ca'||p==='c')return 'a';if(i+1<T.length&&isParticiple(T,i+1))return 'a';if(vlike(T,i-1))return 'à';return null;}
