...
function estQuestion(t,maxMots){
  ...
  function _voisinAuxLong(dl){if(_AUX_LONGS===null){_AUX_LONGS=[];for(var f in FULL_AUX)if(f.length>=4)_AUX_LONGS.push(f);}for(var q=0;q<_AUX_LONGS.length;q++)if(Math.abs(_AUX_LONGS[q].length-dl.length)<=1&&svLev(dl,_AUX_LONGS[q])===1)return true;return false;}
  ...
  function sujFlexVig(T,i){   /* « nous iriez » → irions · « tu irai » → iras. Le sujet donne personne et nombre, la forme écrite
    ...
    if(_lecC.length)lec=_lecC;
    var cible=null,slot=per+nb,lemc=null;
    for(k=0;k<lec.length;k++){
      var f=_CONJ_FIX[lec[k][0]+'|'+lec[k][1]+'|'+slot]||((CONJ_C[lec[k][0]]||{})[lec[k][1]]||{})[slot];
      if(!f)return null;
      if(cible===null){cible=f;lemc=lec[k][0];}else if(cible!==f)return null;
    }
    ...
    if(typeof _lf==='number'&&_lf<1&&_voisinAuxLong(dl))return null;
