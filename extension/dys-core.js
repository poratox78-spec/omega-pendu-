...
(function (global) {
  ...
  function _applyVdc(vd){/* ⭐ 25/09/2026 — PONT VERS LA POLICE DE SON.  est chargé comme script SÉPARÉ par son_panel.js : il ne voit PAS cette fermeture, donc il retombait sur la liste de 582 mots pour le « -ent » des verbes (7 % des cas) alors que la table est déjà chargée ici. On la publie, il la lit si elle est là. */try{ if(typeof self!=="undefined") self.__OMEGA_CJ=(vd&&vd.cj)||null; }catch(e){}BCLF=vd.bclf||null;(vd.v||[]).forEach(function(w){COMMON_VERBS[w]=1;});GENDER_MAP=vd.g||{};GENDER_PURE=vd.gn||{};ADJP=vd.a||{};var cj=vd.cj||{};CONJ_F=cj.f||{};CONJ_C=cj.c||{};_fillReg3pl(CONJ_C,CONJ_F);NOUN_PLURAL={};(vd.gp||[]).forEach(function(w){NOUN_PLURAL[w]=1;});var _GOE={soeur:'f',soeurs:'f',coeur:'m',coeurs:'m',oeuf:'m',oeufs:'m',oeuvre:'f',oeuvres:'f',boeuf:'m',boeufs:'m',voeu:'m',voeux:'m',noeud:'m',noeuds:'m',oeil:'m',moeurs:'f',manoeuvre:'f',manoeuvres:'f',oeillet:'m',oeillets:'m',oesophage:'m',foetus:'m'};for(var _goeK in _GOE)if(GENDER_PURE[_goeK]===undefined)GENDER_PURE[_goeK]=_GOE[_goeK];}   // GENRE noms en œ manquants du lexique gn (débloque « mon soeur »→ma sœur). FP=0, union. Miroir app + Python.
  ...
  function lexicalGender(T,idx){if(!GENDER_MAP)return null;var j,w,g;
    ...
    return null;}
  var MODAL={};("veux veut veulent peux peut peuvent dois doit doivent va vais vas vont faut sais sait aime aimes aiment adore espere souhaite prefere preferent vient viens allons allez laisse laissent ose pour sans afin de devons devez pouvons pouvez voulons voulez").split(/\s+/).forEach(function(w){MODAL[w]=1;});
...
var w=T[i],lw=w.toLowerCase();
    ...
    else if(_INF_GOV[p]||MODAL[p]||_CAUS[p]){/* ⭐ …SAUF UN NOM QUE SEULE LA TABLE ACCENTUÉE CONNAÎT (08/09/2026, miroir Python) : les trois listes de noms homographes de cette règle (NOUN_E, _FLEX_STOP, NOUN_EE) sont DÉSACCENTUÉES — « trace » (f) et « tracé » (m) s'y annulent, donc « et de tracé (y compris…) » devenait « tracer ». La graphie ACCENTUÉE est sous la main : _GACC sait que c'est un nom. Même geste que dans la règle sœur rEer. Mesuré : 0 correction perdue, 12 FP éteints (« le droit de cité »→citer, « la charge de juré »→jurer, « pour mission de protégé »→protéger…), 0 tir sur 14 450 phrases d'UD. */if(_INF_GOV[p]&&/(é|és|ée|ées)$/.test(lw)){var _g5=_GACC[lw];if(_g5==='m'||_g5==='f'){/* ⚠️ RESSERRÉ le 08/09 par la BATTERIE (cf. règle sœur rEer) : intersection avec le tagger, sinon « de rentré » perdait sa correction. */var _t5=posTags(T);if(_t5&&i<_t5.length&&(_t5[i]==='NOUN'||_t5[i]==='PROPN'))return null;}}tgt='inf';}
    ...
    var sugg=forms[tgt];if(tgt==='p2pl'&&cur==='fut1')sugg=inf+'ez';if(tgt==='part'){var _ag=_ppRelit(T,i,sugg);if(_ag)sugg=_ag;}   /* ⭐ 12/09/2026 (lot 2) : participe ACCORDÉ si le contexte le permet */   /* ⭐ 12/09/2026 : le TEMPS ÉCRIT est gardé (« vous souhaiterai » → souhaiterez, pas souhaitez) — miroir Python */
  ...
  function rSon(T,i){var lw=deacc(T[i].toLowerCase());if(lw!=='son'&&lw!=='sont')return null;   // tranché par CE QUI SUIT : son=det+nom sg ; sont=être 3pl+prédicat ; abstention sinon (FP=0)
    ...
    if(i+1<T.length&&pluralSubj&&!NUM_DET[cprev(T,i)]&&!PREP[cprev(T,i)]&&/[sx]$/.test(nxt)
       &&!_SON_INVAR[deacc(nxt)]&&!INVAR_NOUN[deacc(nxt)]&&_wordKnown(/aux$/.test(deacc(nxt))?deacc(nxt).slice(0,-3)+'al':deacc(nxt).slice(0,-1))&&_clauseNoFiniteVerb(T,i,i+1)){   /* ⭐ 11/09/2026 : « son SENS », « son DÉCÈS », « son REFUS » étaient réécrits en rouge — le -s n'est une marque de pluriel que si le singulier existe et que le mot n'est pas un invariable connu (miroir Python) */
  ...
  function _looksPpl(w){if(_isPpl(w))return true;var lw=w.toLowerCase(),d=deacc(lw);if(d.length<3)return false;
    ...
    var d2=/es$/.test(d)?d.slice(0,-2):(/[se]$/.test(d)?d.slice(0,-1):d);if(IRREG_PART[d2]||_PPL_IRR2[d2])return true;   // disparus → disparu, élue → élu
...
var lw=deacc(T[i].toLowerCase());if(lw!=='on'&&lw!=='ont')return null;
    ...
    if(lw==='ont'){var _j=i-1,_st=0;while(_j>=0&&_st<3&&CLITIC[deacc(T[_j].toLowerCase())]){_j--;_st++;}   // miroir Python _subject_before : « ils ont envie », « ils nous ont rien demandé » → sujet pluriel NET, « ont » juste (FP rouge vu le 14/09)
      if(_j>=0){var _bb=false;if(_SEG)for(var _m=_j+1;_m<Math.min(i+1,_SEG.bb.length);_m++)if(_SEG.bb[_m])_bb=true;
        if(!_bb){var _me=_ELIDED_PRON.exec(T[_j].toLowerCase()),_pj=_me?deacc(_me[1]):deacc(T[_j].toLowerCase());if(_pj==='ils'||_pj==='elles')return null;}}}
    ...
    if(!ci){
      ...
      if(i+1<T.length&&_isPpl(T[i+1])){var _rp=svReads(T[i+1]),_pre=false,_nx1=T[i+1].toLowerCase();for(var _q=0;_q<_rp.length;_q++)if(_rp[_q][1]==='ind:pre'&&((((CONJ_C[_rp[_q][0]]||{})['ind:pre']||{})[_rp[_q][2]+_rp[_q][3]])||'').toLowerCase()===_nx1)_pre=true;if(!_pre)return 'ont';}   /* ACCENT-EXACT : svReads lisait « occupés » comme « occupes » et taisait « Sumériens on occupés » → ont */   /* ⭐ 11/09/2026 : « comme on DIT », « duquel on FAIT » — participe MAIS AUSSI présent 3s, sans sujet pluriel à gauche → abstention ; les participes purs (été, occupés, grandi) gardent le rouge. Miroir Python. */
  ...
  function rLeur(T,i){var lw=deacc(T[i].toLowerCase());if(lw!=='leur'&&lw!=='leurs')return null;if(i+1>=T.length)return null;if(vlike(T,i+1))return 'leur';var dn=deacc(T[i+1].toLowerCase());if(INVAR_NOUN[dn])return 'leur';
    /* ⭐ 11/09/2026 : le NOMBRE se lit sur le NOM, pas sur ce qui s'intercale (« leur PLUS grand succès », « leur TRÈS grande efficacité » → leurs, faux) ; on saute adverbes/adjectifs (3 au plus) ; un NUMÉRAL impose le pluriel (« leurs HUIT cartes »). Miroir Python. */
    var jn=i+1,_tgl=posTags(T),_plAdj=false;
    if(_tgl&&jn<_tgl.length&&_tgl[jn]==='NUM')return (jn+1<T.length&&/[sx]$/.test(deacc(T[jn+1].toLowerCase())))?'leurs':null;
    while(_tgl&&jn<T.length-1&&jn<i+4&&(_tgl[jn]==='ADV'||_tgl[jn]==='ADJ')){if(_tgl[jn]==='ADJ'){var _da=deacc(T[jn].toLowerCase());if(/[sx]$/.test(_da)&&_wordKnown(/aux$/.test(_da)?_da.slice(0,-3)+'al':_da.slice(0,-1)))_plAdj=true;}jn++;}   /* le -s de l'adjectif n'est un pluriel que si son singulier existe (« français » → « françai » ✗, mesuré « leur français ») */
    if(jn!==i+1){if(_plAdj)return lw==='leurs'?null:'leurs';   /* témoin pluriel sur l'adjectif sauté (« leurs PREMIERS moie ») : « leurs » est juste, on ne lit pas le nom */
      if(!(_tgl&&jn<_tgl.length&&(_tgl[jn]==='NOUN'||_tgl[jn]==='PROPN')))return null;dn=deacc(T[jn].toLowerCase());if(!_wordKnown(dn))return null;if(INVAR_NOUN[dn])return 'leur';}
    ...
    if(lw==='leurs'&&!/[sx]$/.test(dn)&&rNounPlural(T,jn))return null;
  ...
  function _etElide(w){var d=deacc((w||'').toLowerCase());return d.indexOf("l'")===0||d.indexOf("d'")===0||d.indexOf("qu'")===0||d.indexOf("n'")===0||d.indexOf("s'")===0||d.indexOf("j'")===0||d.indexOf("m'")===0||d.indexOf("t'")===0||d.indexOf("c'")===0;}
function rEt(T,i){var lw=deacc(T[i].toLowerCase());if(lw!=='et'&&lw!=='est')return null;
    ...
    if(!(p==='il'||p==='elle'||p==='on'||p==='c'||p==='ce'||p==='ca'||p==='qui')){
      ...
      if(_j+1<T.length&&(deacc(T[_j+1].toLowerCase()) in NUM_DET))return null;/* ⭐ QUATRE GARDES DE PLUS (08/09/2026), nées de 12 FP mesurés sur les corpus alignés et 5 sur UD, tous du français CORRECT que la branche réécrivait en ROUGE. G4 : une PRÉPOSITION avant le déterminant ⇒ le groupe est un complément, pas un sujet (« de la noblesse et grande bourgeoisie », « entre les FTP et l'Armée ») — doctrine de _npSubject. G3 : « X et demi » est figé. G2 : une CAPITALE après « et » = nom propre (la branche PRONOM avait déjà cette garde, pas la nominale). G1 : si l'« adjectif » est suivi d'un NOM ou d'un déterminant ÉLIDÉ, c'est une coordination de groupes nominaux (« et autres guerres », « et hymne l'ensemble »). Coût mesuré : 0 correction juste perdue. Miroir Python rule_et_est. */if(i>=3&&PREP[deacc(T[i-3].toLowerCase())])return null;var _n2=deacc(T[i+1].toLowerCase());if(_n2==='demi'||_n2==='demie')return null;var _cu=T[i+1].charAt(0),_cv=T[_j].charAt(0);if(_cu!==_cu.toLowerCase()||_cv!==_cv.toLowerCase())return null;if(_etElide(T[_j]))return null;if(_j+1<T.length&&(_etElide(T[_j+1])||_tg[_j+1]==='NOUN'||_tg[_j+1]==='PROPN'))return null;
  ...
  var _E_PPL_STOP={};('cause envie affaire affaires confiance honte hate chance peine conscience connaissance tendance coutume estime importance influence crainte cure grace force partie suite tete course prise charge cesse carte').split(' ').forEach(function(w){_E_PPL_STOP[w]=1;});   // locutions « avoir + nom NU » : jamais un participe
  ...
  var _REFL_CL={me:1,m:1,te:1,t:1,se:1,s:1,nous:1,vous:1},_SUBJ3={il:"",on:"",elle:"e",ils:"s",elles:"es"},_SUBJ12={je:"",tu:"",nous:"s",vous:"s"};
  function _etreSubject(T,i){var tok=T[i-1].toLowerCase(),j=i-2,refl=(tok.indexOf("'")>=0&&["s","m","t"].indexOf(deacc(tok.split("'")[0]))>=0);
    if(j>=0&&_REFL_CL[deacc(T[j].toLowerCase()).replace(/'/g,"")]&&!refl){refl=true;j--;}if(j<0)return [null,refl];if(/^d['’]/.test(T[j].toLowerCase()))return [null,refl];   // « d'elle » = complément, jamais sujet
    var m=_ELIDED_PRON.exec(T[j].toLowerCase()),p=m?deacc(m[1]):deacc(T[j].toLowerCase()).replace(/'/g,"");return [(_SUBJ3[p]!==undefined||_SUBJ12[p]!==undefined)?p:null,refl];}
  function _spos(w){return (SP&&SP.ready&&SP.POS&&SP.POS[w.toLowerCase()])?SP.POS[w.toLowerCase()]:"";}   // POS accent-exact du speller (miroir Python _spos : même asset)
  var _DET_F={la:1,une:1,ma:1,ta:1,sa:1,cette:1},_HOMO_ORANGE=false;   // déterminants féminins sûrs ; _HOMO_ORANGE : orange sur l'homographe adjectif « elle est sèche » (miroir Python — mesuré, Rem tranche)
  function _pplForm(w,suf){var lw=w.toLowerCase(),base=lw.slice(0,-1),c=[base],k=Math.max(base.lastIndexOf("è"),base.lastIndexOf("ê"));   // participe en -é : radical ACCENTUÉ par le lexique (sèche → séché, lève → levé) — miroir Python _ppl_form
    if(k>=0)c.push(base.slice(0,k)+"é"+base.slice(k+1),base.slice(0,k)+"e"+base.slice(k+1));
    for(var q=0;q<c.length;q++)if(SP&&SP.ready&&SP.WORDS&&SP.WORDS.has(c[q]+"er"))return c[q]+"é"+suf;return base+"é"+suf;}
  /* ⭐ BRIQUE 2 de la flexion (14/09/2026, avec Rem) — ORANGE « participe après être à vérifier » : sujet je/tu/nous/vous ou nominal (genre
     absent du texte), ou homographe d'un adjectif (« elle est sèche » / « séchée ») : on propose, l'auteur tranche. Miroir Python rule_e_ppl_vig. */
  function rEPplVig(T,i){var w=T[i],lw=w.toLowerCase(),dl=deacc(lw);if(i<2||lw.indexOf("'")>=0||!/e$/.test(dl)||/é$/.test(lw)||/ée$/.test(lw)||dl.length<4||w.charAt(0)!==w.charAt(0).toLowerCase())return null;
    if(!AUX_ETRE[deacc(T[i-1].toLowerCase()).split("'").pop()])return null;if(T[i-1].toLowerCase().indexOf("à")>=0||T[i-1].charAt(0)!==T[i-1].charAt(0).toLowerCase())return null;
    if(!COMMON_VERBS[deacc((w.slice(0,-1)+"er").toLowerCase())])return null;if(PREP[dl]||MODAL[dl]||NOUN_E[dl]||_E_PPL_STOP[dl])return null;
    var p2=deacc(T[i-2].toLowerCase());if(p2==="nord"||p2==="sud")return null;if(_isPpl(w)||IRREG_PART[dl.slice(0,-1)])return null;var pv=_spos(lw);if(pv.indexOf("V")<0)return null;
    var sr=_etreSubject(T,i),sbj=sr[0],refl=sr[1],homo=pv.indexOf("A")>=0;
    if(homo&&!_HOMO_ORANGE)return null;if(!homo&&sbj!==null&&_SUBJ3[sbj]!==undefined)return null;if(!homo&&sbj===null&&!refl&&GENDER_PURE[dl]!==undefined)return null;
    var suf;if(sbj!==null&&_SUBJ3[sbj]!==undefined)suf=_SUBJ3[sbj];else if(sbj!==null)suf=_SUBJ12[sbj];else{var g=GENDER_PURE[p2];if(g===undefined&&i>=3&&_DET_F[deacc(T[i-3].toLowerCase())])g="f";suf=(g==="f")?"e":"";}
    var sg=_emit(w,function(x){return _pplForm(x,suf);},true);if(sg===null)return null;return {sugg:ckeepcase(w,sg),vig:1};}   // _pplForm : accent du radical restauré par le lexique (sèche → séché), comme le rouge
  function rEPpl(T,i){var w=T[i],lw=w.toLowerCase(),dl=deacc(lw);   // AUXILIAIRE + verbe au PRÉSENT en -e → PARTICIPE en -é (« ont trouve »→trouvé). Le dys écrit la forme qu'il ENTEND ; après un auxiliaire une forme FINIE est structurellement impossible. La règle é/er ne voyait que -é↔-er, jamais le présent nu.
    ...
    var _sr=(i>0&&AUX_ETRE[deacc(T[i-1].toLowerCase()).split("'").pop()])?_etreSubject(T,i):[null,false],_sbj=_sr[0],_pv=_spos(lw);
    var _sure=(_pv.indexOf('V')>=0&&_pv.indexOf('A')<0&&_sbj!==null&&_SUBJ3[_sbj]!==undefined&&!(i>=2&&/^(nord|sud)$/.test(deacc(T[i-2].toLowerCase()))));   // brique 2 : forme verbale pure + sujet 3e pers. → participe SÛR
    var _etrePp=(i>0&&AUX_ETRE[deacc(T[i-1].toLowerCase()).split("'").pop()]&&(_PPL_ETRE_VERBES[dl]||_sure));
    var _auxBareA=(i>0&&deacc(T[i-1].toLowerCase()).split("'").pop()==='a');   // « a » NU = homographe de « à » : gardes nom conservées là seulement (miroir Python _aux_bare_a)
    if(!_etrePp&&(_E_PPL_STOP[dl]||(_auxBareA&&dl==='cote')))return null;   // idiomes « avoir + nom nu » ; « a cote » = à côté. Test de Rem : « j'ai pris » → participe
    if((GENDER_PURE[dl]!==undefined||NOUN_E[dl])&&!_etrePp&&_auxBareA){var nx=(i+1<T.length)?deacc(T[i+1].toLowerCase()):'';   // NOM homographe (commande, place, garde, écoute…) : après un auxiliaire un nom NU n'existe qu'en LOCUTION ; un vrai complément exige un DÉTERMINANT, qui est AUDIBLE donc fiable
    ...
    var _suf='';
    if(_etrePp&&_sbj!==null)_suf=(_SUBJ3[_sbj]!==undefined)?_SUBJ3[_sbj]:(_SUBJ12[_sbj]||'');   // sujet lu par _etreSubject (pronom, réfléchi, élidé)
    var _sg=_emit(w,function(x){return _pplForm(x,_suf);},true);   // radical accentué par le lexique (« il est lève » → levé, pas « lèvé ») — miroir Python _ppl_form   // PRIMITIVE PARTAGÉE, mode « forme déjà établie fautive par le contexte » : « il a repare » -> « réparé » (et non « reparé », qui existe mais n'est pas le mot voulu)
    ...
    if(CAUX[nd]||nd==='sont'||nd==='est'){   /* ⭐ 12/09/2026 : ce qui suit l'AUXILIAIRE tranche « ce sont » / « se sont » — un PARTICIPE ne suit jamais « ce sont » (groupe nominal) ; sujet exigé devant, sinon ce serait « ils sont » (miroir Python rule_ce_se) */
      if(lw==='ce'){var _j=i+2;while(_j<T.length&&_j<=i+4&&PPMID[deacc(T[_j].toLowerCase())])_j++;
        if(_j<T.length&&_isPpl(T[_j])){var _tg0=posTags(T),_p0=_prevPron(T,i);
          if(_p0==='il'||_p0==='elle'||_p0==='on'||_p0==='ils'||_p0==='elles'||_p0==='je'||_p0==='tu'||_p0==='qui'||(i>0&&_tg0&&i-1<_tg0.length&&(_tg0[i-1]==='NOUN'||_tg0[i-1]==='PROPN')))return ckeepcase(T[i],'se');}}
      return null;}
  ...
  function rMaiMais(T,i){if(deacc(T[i].toLowerCase())!=='mai')return null;
    ...
    if(_SEG&&i<_SEG.dig.length&&_SEG.dig[i])return null;              // ⭐ UN NOMBRE JUSTE AVANT = une DATE (08/09/2026, miroir Python). Le tokeniseur jette les chiffres : « le 6 mai 1928 » arrive comme [le, mai] et la garde de date (liste de MOTS) tient — jusqu'à ce qu'un nom s'intercale (« le sport 6 mai 1928 »), et « mai » devenait « mais » sur du français correct. _seg_info calcule DÉJÀ ce drapeau pour les déterminants (« le 25 mars »). Mesuré : 0 correction perdue, 1 FP éteint, 0 tir sur 14 450 phrases d'UD.
  ...
  function rElide(T,i){var w=T[i],lw=w.toLowerCase();if(ELIDE_STOP[lw])return null;if(/^(qu|[jnmtsdcl])'/.test(lw))return null;   /* ⭐ 13/09/2026 : « élision inversée » couvre ces préfixes ; son orange ou son abstention (nom propre) ne doivent pas être réappliqués en rouge ici. Miroir Python rule_elide. */for(var pre in ELIDE){if(lw.indexOf(pre)===0){var rest=w.slice(pre.length),c0=rest.charAt(0);if(rest&&c0===c0.toLowerCase()&&c0!==c0.toUpperCase()&&ECONS.indexOf(deacc(c0.toLowerCase()))>=0)return ckeepcase(w,ELIDE[pre]+' '+rest);return null;}}return null;}
  ...
  function _verb3pOnly(w){if(!w)return false;var r=svReads(w),k,p=false,s=false;for(k=0;k<r.length;k++){if(r[k][2]==='3'&&r[k][3]==='p')p=true;if(r[k][2]==='3'&&r[k][3]==='s')s=true;}return p&&!s;}   /* ⭐ 11/09/2026 : 3e pluriel sans lecture 3e singulier (« ont », « mangent ») = témoin de pluriel. Miroir Python _verb_3p_only. */
...
var pn=svSubject(T,i);if(!pn)return null;var per=pn[0],nb=pn[1];
    if(per==='3'&&nb==='s'&&_V3PL_SURE[deacc(T[i].toLowerCase())]){var _pp=i>0?deacc(T[i-1].toLowerCase()):'';if((_pp==='ne'||_pp==='n')&&i>1)_pp=deacc(T[i-2].toLowerCase());var _me2=_ELIDED_PRON.exec(_pp);if(_me2)_pp=deacc(_me2[1]);if(_pp==='il'||_pp==='elle')return null;}   /* ⭐ 11/09/2026 : pronom ÉLIDÉ (« lorsqu'il sont évincés ») — même garde. Miroir Python. */   // « il/elle » + verbe SÛR 3pl → rIlIls fixe le pronom (pas le verbe)
    ...
    var _lfa=_lemFreq(lem);if(typeof _lfa==='number'&&_lfa<1){var _vxa=_voisinAuxLong(_dsv);if(_vxa){var _rva=svReads(_vxa);for(k=0;k<_rva.length;k++){if(_rva[k][0]==='etre'||_rva[k][0]==='avoir'){var _fa=((CONJ_C[_rva[k][0]]||{})[_rva[k][1]]||{})[per+nb];if(_fa&&_fa.toLowerCase()!==T[i].toLowerCase())return ckeepcase(T[i],_fa);}}return null;}}   /* ⭐ 12/09/2026 : « ils somme » → sont, pas « somment » — lemme RARE à une lettre d'une forme longue d'être/avoir → cet auxiliaire conjugué (miroir Python rule_accord_sv) */
    /* ⭐ LE MODE AVANT LE TEMPS (08/09/2026, miroir Python rule_accord_sv) : `reads[0][1]` n'est pas « le temps tapé » mais le PREMIER ORDRE DU LEXIQUE. « Il faut que je disiez » a pour lectures {ind:imp, sub:pre} → le repli rendait « disais » au lieu de « dise ». Sur DÉCLENCHEUR FERMÉ de subjonctif, le mode passe devant. Mesuré : banc Bescherelle subjonctif 73,5 → 99,0 %, banc entier 92,4 → 96,8 %, 0 phrase correcte cassée, 0 verdict changé sur les 381 textes à déclencheur des 48 866 corpus. PLACEBO (mode d'abord SANS déclencheur) : même gain sur le subjonctif mais 8 autres cases CASSÉES — c'est la liste fermée qui porte la garde. */var _sbm=null;for(k=0;k<reads.length;k++)if(reads[k][1].indexOf('sub')===0){_sbm=reads[k][1];break;}var mt=(_sbm&&_subjTrigger(T,i))?_sbm:(mts['ind:pre']?'ind:pre':reads[0][1]);if(mt==='ind:pas')return null;var slots=(CONJ_C[lem]||{})[mt];if(!slots)return null;var sugg=slots[per+nb];if(!sugg)return null;
    if(!svAgrees(svReads(sugg),per,nb))return null;return sugg;}
  var _DET_PL={le:'les',la:'les',un:'des',une:'des',ce:'ces',cet:'ces',cette:'ces',mon:'mes',ma:'mes',ton:'tes',ta:'tes',son:'ses',sa:'ses',notre:'nos',votre:'vos'};   // pas « leur » : rLeur le porte déjà
  function rDetNumber(T,i){var lw=deacc(T[i].toLowerCase());if(!_DET_PL[lw]||T[i].indexOf("'")>=0||i+2>=T.length)return null;if(i>0&&/^(et|ou|ni)$/.test(deacc(T[i-1].toLowerCase())))return null;   /* conjoint (« l'hébergement et la restaurations ont ») : le pluriel vient de la coordination — miroir Python */   /* ⭐ 11/09/2026 : la case manquante du triangle dét/nom/verbe — « le MAÇONS ONT » → les ? (ORANGE). Miroir Python rule_det_number. */
    var n=T[i+1],c0=n.charAt(0);if(!/[A-Za-zÀ-ÿœŒæÆ]/.test(c0)||c0!==c0.toLowerCase())return null;var dn=deacc(n.toLowerCase());
    if(dn.length<4||!/[sx]$/.test(dn)||_INVAR_S[dn]||_SG_STOP[dn]||NOUN_PL_STOP[dn])return null;
    var sg=/aux$/.test(dn)?dn.slice(0,-3)+'al':dn.slice(0,-1);if(!_wordKnown(sg))return null;
    if(!_verb3pOnly(T[i+2]))return null;return ckeepcase(T[i],_DET_PL[lw]);}
  function rIlIls(T,i){var _el=_ELIDED_PRON.exec(T[i].toLowerCase()),lw=_el?deacc(_el[1]):deacc(T[i].toLowerCase());if(lw!=='il'&&lw!=='elle')return null;   /* ⭐ 11/09/2026 : pronom ÉLIDÉ (« lorsqu'il sont » → lorsqu'ils). Miroir Python. */   // AUDIBILITÉ sur le SUJET : « il/elle » + verbe SÛR 3pl → « s » de ils/elles muet (lâché par le dys), verbe audible fiable → corriger le PRONOM. « il sont »→« ils sont ». FP=0. Miroir Python/app.
    ...
    var _s=lw==='il'?'ils':'elles';return _el?T[i].slice(0,T[i].length-_el[1].length)+_s:ckeepcase(T[i],_s);}
  ...
  function rPostpose(T,i){var lw=T[i].toLowerCase();if(lw.indexOf("'")>=0||lw==='à')return null;   // accord SUJET-VERBE à sujet POSTPOSÉ (inversion) : ordre changé → scan AVANT. FP=0 mesuré. Miroir Python rule_accord_postpose.
    ...
    return sugg;}
  function _temoinApresVerbe(T,tg,i,nb){var _dv=deacc(T[i].toLowerCase()),_nx=i+1<T.length?deacc(T[i+1].toLowerCase()):'';if(_dv==='sont'&&nb==='s'){if(_POST_PL[_nx])return true;if(i+2<T.length&&(_isPpl(T[i+1])||/(és|ées)$/.test(T[i+1].toLowerCase())||(tg&&tg[i+1]==='VERB'&&/s$/.test(_nx)))&&_POST_PL[deacc(T[i+2].toLowerCase())])return true;}if(_dv==='est'&&nb==='p'&&(_POST_SG[_nx]||_nx.slice(0,2)==="l'"))return true;return false;}   /* ⭐ 11/09/2026 (restes frgec, lot 2) : ce qui SUIT le verbe témoigne du nombre du VRAI sujet — A « la passion … sont DES thèmes » (attribut pluriel, sujet parsé singulier → énumération à gauche) ; B « Chaque année sont organisés DES milliers » (sujet postposé) ; F « Les impudents est LE premier roman » (attribut singulier, sujet parsé pluriel = titre). Partagée par rAccordSVnoun (rouge) et sujFlexVig (orange). Miroir Python _temoin_apres_verbe. */
function rAccordSVnoun(T,i,vig){var lw=T[i].toLowerCase();if(lw.indexOf("'")>=0||lw==='à')return null;
    ...
    if(_temoinApresVerbe(T,tg,i,nb))return null;   /* ⭐ 11/09/2026 (restes frgec, lot 2) : ce qui suit le verbe témoigne du vrai sujet (A/B/F, cf. _temoinApresVerbe). Miroir Python. */
  ...
  function rAccordSVrelatif(T,i){var lw=T[i].toLowerCase();if(lw.indexOf("'")>=0||lw==='à')return null;
    ...
    var _dv=deacc(T[i].toLowerCase());if((_dv==='est'||_dv==='sont')&&i+1<T.length&&(_isPpl(T[i+1])||/(é|ée|és|ées)$/.test(T[i+1].toLowerCase())||tg[i+1]==='VERB')){var _vn=_dv==='sont'?'p':'s',_pnb=/s$/.test(deacc(T[i+1].toLowerCase()))?'p':'s';if(_pnb===_vn&&_vn!==nb)return null;}   /* ⭐ 11/09/2026 (restes frgec, lot 2) : « la sexualité qui sont FAITES » — le participe s'accorde avec le verbe et contredit l'antécédent le plus proche → l'antécédent réel est plus à gauche. Miroir Python. */
  ...
  function rAccordVerbCoord(T,i){   // récupère le sujet via le VERBE COORDONNÉ (idée Rem : le sujet est dans le verbe frère) — miroir Python rule_accord_verb_coord, FP=0
    ...
    if(_SEG&&ci+1<_SEG.dig.length&&(_SEG.dig[ci]||_SEG.dig[ci+1]))return null;   /* ⭐ 11/09/2026 (restes frgec, lot 2) : « entre 2006 et 2016 sont parues » — un NOMBRE touche la conjonction : « et » coordonne des nombres, pas des verbes. Miroir Python. */
  ...
  function rAccordSVcoord(T,i){var lw=T[i].toLowerCase();if(lw.indexOf("'")>=0||lw==='à')return null;
    ...
    for(m=lo;m<i;m++){var dm=deacc(T[m].toLowerCase());
      ...
      if(T[m].toLowerCase().indexOf("'")>=0)return null;
      if(tg[m]==='VERB'||tg[m]==='AUX'||PREP[dm])return null;if(svReads(T[m]).length&&!(m>lo&&(NUM_DET[deacc(T[m-1].toLowerCase())]||tg[m-1]==='DET')))return null;   /* ⭐ 11/09/2026 : verbe manqué par le tagger dans la zone sujet (« La vérité ÉCLATE et Georges est ») — lecture verbale finie sans déterminant devant → verbe, pas conjoint. Miroir Python. */
...
var QW=/^(est-ce|qu['’]est|qu\s+est[-\s]ce|où|comment|pourquoi|quand|combien|quel|quelle|quels|quelles|lequel|laquelle)(?![a-zà-ÿœ])/i;   // interrogatifs FORTS en tête (lookahead car \b casse après « où »)
...
var QEQ=/^\s*est[-\s]ce\s+(?:que|qui|qu['’])/i;   // « est-ce QUI » : même tête que « est-ce que » (Rem, 14/09)
var QEQ3=/^\s*est[-\s]ce(?!\s+qu)(?![a-zà-ÿœ])/i;   // « est-ce possible ? » : en TETE, « est-ce » n'a pas d'emploi non interrogatif
var QEQ2=/^\s*[a-zà-ÿœ']+\s+est[-\s]ce\s+(?:que|qui|qu['’])/i;   // « qui est-ce qui », « pourquoi est-ce que »
...
var QSEUL=/^(qu['’]est|qu\s+est[-\s]ce|comment|pourquoi|combien)(?![a-zà-ÿœ])/i;   // « qu est ce qui » sans apostrophe (graphie dys)
...
function estQuestion(t,maxMots){
  ...
  if(QPRON.test(t) && t.indexOf(',')<0 && QVERBAL[tag(1)]===1) return true;
  if(/^\s*Qui(?![a-zà-ÿœ'’])/.test(t) && t.indexOf(',')<0 && mots.length<=8 && QVERBAL[tag(1)]===1) return true;   // « Qui est là », « Qui vient demain » : « Qui » MAJUSCULE en tête (vraie tête de phrase) + verbe, ≤ 8 mots, sans virgule (Rem, 14/09 : « plus dur »). Mesuré sur question_bench : sans la majuscule, les relatives en tête de fragment (« qui stockent les valeurs… ») font tomber la précision de 91 à 48,6 % ; avec : 91,30 %, +3 questions, 0 faux de plus   // « Lequel est le plus grand ? » ; le relatif, lui, porte un clitique (« laquelle lui repond »)
  ...
  /* ⭐ NOM FÉMININ EN -ée ÉCRIT -é APRÈS DÉTERMINANT FÉMININ (13/09/2026, plan ⑤-c B — miroir Python rule_fem_ee) : « la cheminé »,
     « une fumé », « la vu ». Liste close, déterminant féminin non ambigu, UD 0. */
  function rFemEe(T,i){if(i===0)return null;if(!_DET_FEM_SUR[deacc(T[i-1].toLowerCase())])return null;var w=T[i];if(!/^[a-zà-ÿœæ]+$/.test(w))return null;
    var f=_FEM_EE[w];if(!f)return null;if(_SEG&&_SEG.hy&&(_SEG.hy[i]||_SEG.hy[i+1]))return null;return f;}
  var _PRON_INF_SLOT={je:"1s",tu:"2s",il:"3s",elle:"3s",on:"3s",qui:"3s",ils:"3p",elles:"3p"},_PRON_INF_CLIT={ne:1,n:1,me:1,m:1,te:1,t:1,se:1,s:1,le:1,la:1,les:1,l:1,lui:1,leur:1,y:1,en:1},
      _PRON_INF_STOP={a:1,de:1,pour:1,sans:1,afin:1,en:1,par:1,avant:1,apres:1,sur:1,avec:1,chez:1,contre:1,vers:1,dans:1,sous:1,entre:1,que:1,qu:1,faut:1,peut:1,peux:1,doit:1,dois:1,va:1,vais:1,vas:1,veut:1,veux:1};
  /* ⭐ FLEXION PAR LE CONTEXTE, brique 1 (14/09/2026) : INFINITIF juste après un pronom sujet (« il reculer », « qui protéger ») → forme
     conjuguée, ORANGE (imparfait proposé — 6 cas gold sur 9 —, présent dans l'astuce). Recensé : UD 1 motif (« sur qui compter »), exclu par
     la préposition avant le pronom. nous/vous exclus (objets possibles). Miroir Python rule_pron_inf. */
  function rPronInf(T,i){var w=T[i],lw=deacc(w.toLowerCase());if(!/er$/.test(lw)||lw.length<4||w!==w.toLowerCase()||!CONJ_C[lw])return null;
    var z=i-1;while(z>=0&&_PRON_INF_CLIT[deacc(T[z].toLowerCase()).replace(/'/g,'')])z--;if(z<0)return null;
    var _el=_ELIDED_PRON.exec(T[z].toLowerCase()),p=_el?deacc(_el[1]):deacc(T[z].toLowerCase()).replace(/'/g,''),slot=_PRON_INF_SLOT[p];if(!slot)return null;   // « qu'il aimer » : pronom dans le token élidé
  ...
  function desingularizeNoun(n){var dn=deacc(n.toLowerCase()),cands=[];   // inverse ancré : -aux→-al, -x→∅, -s→∅ ; ne renvoie QUE si la forme sing. est un NOM confiant
    ...
    // ⛔ finale en voyelle accentuée AUTRE que -é = pas un mot français : « Le congrès »→« congrè »
    // en AUTO (mesuré 07/09/2026 sur gold Wikipédia). L'ancre NOUN_POST est DÉACCENTUÉE, donc
    // « congré » y trouve « congre » et passe. Même parade que pour « apex » : la FORME tranche.
    // Miroir EXACT de correcteur_probe._singularize_noun.
    for(var k=0;k<cands.length;k++){var _c=cands[k];if(_c&&'àâäèêëìîïòôöùûü'.indexOf(_c.charAt(_c.length-1))>=0)continue;
      var p=NOUN_POST.get(deacc(_c.toLowerCase()));if(p&&p[0]>=PL_TAU_M&&p[1]<PL_EPS_M)return _c;}return null;}
  ...
  function rNounSing(T,i){if(!NOUN_POST)return null;
    ...
    var nx=i+1<T.length?T[i+1]:'';
    var _dj=_pre?i:((i>=2&&_ADJ_ANTE[deacc(T[i-1].toLowerCase())]&&_SING_DET[deacc(T[i-2].toLowerCase())])?i-2:i-1);   /* index du déterminant ; sauf CONJOINT (« l'hébergement et la restaurations ont ») : le pluriel du verbe vient de la coordination */
    if(!_pre&&_verb3pOnly(nx)&&!(_dj>=1&&/^(et|ou|ni)$/.test(deacc(T[_dj-1].toLowerCase()))))return null;   /* ⭐ 11/09/2026 : « le MAÇONS ont » — le verbe qui suit est un 3e pluriel : deux témoins contre le déterminant → abstention (miroir Python, cohérent avec rOn). */
    ...
    if(/s$/.test(dn)&&NOUN_PLURAL[dn]&&GENDER_PURE[dn.slice(0,-1)]!==undefined){                       // VOIE RELÂCHÉE : pluriel homographe verbal (« la boites »=boiter 3sg) tranché par dét sing + tagger + lexique
      ...
      if('àâäèêëìîïòôöùûü'.indexOf(n.charAt(n.length-2))>=0)return null;                                // même garde de FORME que desingularizeNoun : « congrès »→« congrè » n'est pas un mot
  ...
  function rDeselide(T,i){var w=T[i],lw=w.toLowerCase();if(lw==="m'sieur"||lw==="m'dame"||lw==="m'ame"||lw==="n'roll"||lw==="c'te")   /* ⚠️ LISTE UNIQUE (audit 2026-08-11) : rElide et rDeselide font le MÊME travail et avaient DEUX listes d'exceptions différentes — « n'roll » n'était que dans celle de rElide. Mesuré : « rock n'roll » -> « rock ne roll ». Miroir de _ELIDE_STOP (Python). */return null;   // élision inversée dys → rétablir (FP=0)
    ...
    var r=_deselide(T,i,pre,rest);return r?r[0]:null;}   // ⭐ 13/09/2026 : suggestion décidée par _deselide ; le palier passe par _tierOf (famille « élision inversée »)
  /* ⭐ 13/09/2026 — MIROIR de correcteur_probe._deselide : ROUGE là où le mot complet est sûr ; ORANGE avec le mot MANQUANT quand la grammaire
     le montre (« elle s'mariée » → s'est mariée, « d'dure écorce » → d'une dure) ; ORANGE sans l'article en double (« de l'pétrole » → pétrole) ;
     RIEN sur un nom propre (« Ben M'barek »). Mesuré avant : vrai écrit dys 5 tirs / 0 juste, tous rouges ; UD 16 950 phrases : 0 tir.
     Rend [suggestion, orange] ; pre/rest facultatifs (rDeselide les a extraits, _tierOf non). */
  var DESEL_CLIT={ne:1,me:1,te:1,se:1,le:1,la:1,les:1,lui:1,leur:1,y:1,en:1,nous:1,vous:1},DESEL_DET={du:1,des:1,de:1,le:1,la:1,les:1,un:1,une:1,au:1,aux:1},DESEL_SUJ={il:['s','m'],elle:['s','f'],on:['s','m'],ils:['p','m'],elles:['p','f']},DESEL_LOC={heure:1,heures:1,matin:1,foi:1,humeur:1,gre:1,grace:1,coeur:1,'cœur':1},DESEL_INF={va:1,vais:1,vas:1,allons:1,allez:1,vont:1,faut:1,peut:1,peux:1,pouvons:1,pouvez:1,peuvent:1,veut:1,veux:1,voulons:1,voulez:1,veulent:1,doit:1,dois:1,devons:1,devez:1,doivent:1,pour:1,sans:1,de:1,aller:1,pouvoir:1,vouloir:1,devoir:1};
  function _deselide(T,i,pre,rest){var w=T[i];
    if(pre==null){var lw=w.toLowerCase();if(lw==="m'sieur"||lw==="m'dame"||lw==="m'ame"||lw==="n'roll"||lw==="c'te")return null;var m=lw.match(/^(qu|[jnmtsdcl])'(.+)$/);if(!m)return null;pre=m[1];rest=w.slice(pre.length+1);var c0=rest.charAt(0);
      if(!c0||DESELV[c0.toLowerCase()]||c0.toLowerCase()===c0.toUpperCase()||c0===c0.toUpperCase())return null;}
    var dr=deacc(rest.toLowerCase()),nom=(GENDER_PURE[dr]==='m'||GENDER_PURE[dr]==='f');
    if(w.charAt(0)!==w.charAt(0).toLowerCase()&&!(nom||svReads(rest).length||_isPpl(rest)||_inf1(rest)!=null||DESEL_CLIT[dr]||DESEL_DET[dr]))return null;   // nom propre : « Ben M'barek », « N'djili »
    var prev=i>0?deacc(T[i-1].toLowerCase()):'';
    if(pre==='l'){var g=GENDER_PURE[dr]||GENDER_MAP[dr];if(g!=='m'&&g!=='f')return null;
      if(prev==='du'||prev==='des'||prev==='au'||prev==='aux'||(prev==='de'&&g==='m'))return [ckeepcase(w,rest),true];   // déterminant en double : « du l'département », « de l'pétrole »
      return [ckeepcase(w,(g==='m'?'le':'la')+' '+rest),false];}
    if(pre==='d'){if(DESEL_DET[dr])return [ckeepcase(w,rest),true];   // « d'du » : déterminant en double
      if(!nom&&i+1<T.length){var nx=deacc(T[i+1].toLowerCase()),gn=GENDER_PURE[nx];if((gn==='m'||gn==='f')&&!DESEL_LOC[nx])return [ckeepcase(w,(gn==='m'?"d'un ":"d'une ")+rest),true];}   // « d'dure écorce » → d'une dure écorce
      return [ckeepcase(w,'de '+rest),false];}
    if((pre==='j'||pre==='n'||pre==='m'||pre==='t'||pre==='s')&&!DESEL_CLIT[dr]&&(!_isFinite(rest)||/(é|ée|és|ées)$/.test(rest.toLowerCase()))){   // finale ACCENTUÉE de participe : prime sur la lecture conjuguée (déaccentués, « mariés » = « tu maries »)
      if(pre==='s'){var inf=_inf1(rest),suj=DESEL_SUJ[prev];if(inf!=null&&DESEL_INF[prev]&&deacc(inf)!==dr)return [ckeepcase(w,'se '+inf),true];   // « il faut s'marié », « elle va s'mariée » → se marier (-é mis pour -er)
        if(_isPpl(rest)||(inf!=null&&deacc(inf)===dr&&suj)){var part=(suj&&inf!=null)?_ppAccord(inf.slice(0,-2)+'é',suj[0],suj[1]):rest;
          return [ckeepcase(w,((suj&&suj[0]==='p')?'se sont ':"s'est ")+part),true];}}   // « elle s'mariée » → s'est mariée
      else if(_isPpl(rest))return [ckeepcase(w,DESEL[pre]+' '+rest),true];}   // « j'mangé » : un auxiliaire est tombé — orange
    return [ckeepcase(w,DESEL[pre]+' '+rest),false];}
  ...
  var PPMID={ne:1,n:1,pas:1,plus:1,jamais:1,y:1,en:1,se:1,s:1,deja:1,toujours:1,aussi:1,bien:1,encore:1,tous:1,toutes:1,tout:1,tres:1,trop:1,peu:1,souvent:1,vraiment:1,assez:1,plutot:1};   // ⭐ 12/09/2026 (lot 2) : adverbes de degré (« sont TRÈS fréquentés »)
  var PPA_AUXP={ont:1,avons:1,avaient:1,avions:1,auront:1,aurons:1,auraient:1,aurions:1,aient:1,ayons:1,eurent:1,eumes:1};   // avoir PLURIEL audible devant « été » (miroir _PP_AVOIR_P)
  ...
  /* [être / avoir été] + IMPARFAIT → PARTICIPE — fait STRUCTUREL mesuré (miroir Python rule_aux_imparfait). Le dys écrit ce qu'il ENTEND : après « est », la finale /e/ du participe s'écrit aussi bien « situait » que « situé ». Une forme FINIE ne peut pas suivre un auxiliaire — la construction EST la garde, comme pour « j'est ». Mesuré (data_local/imp_pp.js) : le cadre [être/avoir été] + une forme qui n'est QU'un imparfait ou un conditionnel apparaît 0 fois sur 4 631 phrases correctes (2 573 cadres) et 15 fois dans le texte dys écrit. Périmètre 1er groupe : c'est là qu'est TOUTE la population, parce que c'est là que l'imparfait et le participe sonnent pareil. */
  var _IMP_PP_ETRE={};('es est etaient etait etes furent fut sera seront soit sommes sont suis serait seraient seras serez serons fus fumes futes soient sois').split(' ').forEach(function(x){_IMP_PP_ETRE[x]=1;});
  function rAuxImparfait(T,i){
    if(i===0)return null;var w=T[i],lw=w.toLowerCase();
    if(lw.indexOf("'")>=0||w.charAt(0)!==w.charAt(0).toLowerCase())return null;
    var rd=svReads(w),k;if(!rd.length)return null;
    for(k=0;k<rd.length;k++)if(rd[k][1]!=='ind:imp'&&rd[k][1]!=='cnd:pre')return null;
    var lem=rd[0][0];for(k=1;k<rd.length;k++)if(rd[k][0]!==lem)return null;
    if(!/er$/.test(lem)||!COMMON_VERBS[deacc(lem)])return null;
    var p1=deacc(T[i-1].toLowerCase()).split("'").pop();
    if(p1==='ete'){if(i<2)return null;var p2=deacc(T[i-2].toLowerCase()).split("'").pop();
      if(!CAUX[p2]&&!_IMP_PP_ETRE[p2])return null;}
    else if(!_IMP_PP_ETRE[p1])return null;
    /* ⚠️ LE LEMME EST DÉSACCENTUÉ (CONJ_C n'a aucune clé accentuée) : « creer », « representer ». L'accent vit dans les FORMES que les tables génèrent — on prend le présent 3s (« crée », « représente », « lève ») et on le passe à _pplForm, écrit exactement pour ça (il rend « levé », pas « lèvé »). Sans ce détour : « crèaient » → *creé*. */
    var pres=((CONJ_C[lem]||{})['ind:pre']||{})['3s'];
    if(!pres||!/e$/.test(pres))return null;   /* table incomplète (reléguer, édifier…) → abstention */
    var sb=_etreSubject(T,i),suf=(sb&&sb[0]!=null)?(_SUBJ3[sb[0]]!==undefined?_SUBJ3[sb[0]]:(_SUBJ12[sb[0]]||'')):'';
    var sugg=_pplForm(pres,suf);
    return sugg.toLowerCase()!==lw?ckeepcase(w,sugg):null;}
  function rPpEtre(T,i){var lw=T[i].toLowerCase();if(lw.indexOf("'")>=0)return null;
    ...
    var a=-1,k,_comp=false;for(k=i-1;k>=0&&k>i-4;k--){var dk=deacc(T[k].toLowerCase());
      if(dk==='ete'||dk==='etait'){for(var k2=k-1;k2>=0&&k2>k-4;k2--){var d2=deacc(T[k2].toLowerCase());if(AVOIR_AUX[d2]||AVOIR_JE[T[k2].toLowerCase()]){a=k2;_comp=true;break;}if(PPMID[d2])continue;break;}if(_comp)break;if(dk==='ete')return null;}   /* ⭐ 12/09/2026 (lot 2) : « ont été dégoûté », « a était raconté » → l'auxiliaire AVOIR porte le sujet (miroir Python) */
      if(PPE_AUX[dk]){a=k;break;}if(PPMID[dk])continue;return null;}
    if(a<0)return null;var auxNum=_comp?(PPA_AUXP[deacc(T[a].toLowerCase())]?'p':'s'):(PPE_AUXP[deacc(T[a].toLowerCase())]?'p':'s');var _a1ok=!!_A1AUXPL[deacc(T[a].toLowerCase())]||(_comp&&!!PPA_AUXP[deacc(T[a].toLowerCase())]);
    ...
    var info=null,sk=-1;if(_comp&&AVOIR_JE[T[a].toLowerCase()]){info=PPE_SUBJ['je'];sk=a;}   // « j'ai été surpris » : le sujet est dans le token de l'auxiliaire
    if(!info)for(k=a-1;k>=0&&k>a-3;k--){var d2=deacc(T[k].toLowerCase());if(d2==='ne'||d2==='n')continue;info=PPE_SUBJ[d2];sk=k;break;}
    ...
    var num=info[0],gen=info[1];if(num!==auxNum){if(num==='s'&&auxNum==='p'&&_a1ok&&(deacc(T[sk].toLowerCase())==='il'||deacc(T[sk].toLowerCase())==='elle'))num='p';else return null;}   /* ⭐ 12/09/2026 (lot 2) : « il sont dégouté » — le -s du pronom est tombé, l'aux audible est fiable (miroir Python) */var _refl=(deacc(T[sk].toLowerCase())==='se')||(sk>=1&&PPE_SUBJ[deacc(T[sk-1].toLowerCase())]);if(_refl&&i+1<T.length){var _tgn=posTags(T);if(_tgn&&i+1<_tgn.length&&(_tgn[i+1]==='NOUN'||_tgn[i+1]==='DET'))return null;}   // pronominal réfléchi COI + COD après → PP INVARIABLE
  ...
  function _npSubject(T,tg,a){if(deacc(String(T[a]||'').toLowerCase()).indexOf("qu'")===0)return null;   /* MARQUEUR FUSIONNE DANS LE VERBE : « les possibilites qu'offrent les domaines » — le token EST « qu'offrent », le balayage part de a-1 et ne le voit jamais. Un verbe en « qu' » est celui d'une RELATIVE/SUBORDONNEE : sujet a DROITE (inversion) ou le relatif lui-meme. Remonter a gauche rend un sujet FAUX → abstention (FP=0). Mesure par dictee/ponct_morpho_probe.py. */var lo=0,j;if(_SEG){for(j=a;j>0;j--){if(j<_SEG.bb.length&&_SEG.bb[j]){lo=j;break;}}}
    ...
    if(_pg&&a-1>=lo&&/^[A-ZÀ-Þ]/.test(T[a-1])){var _co=false;
      ...
      var _pv=(a-2>=lo)?deacc(T[a-2].toLowerCase()):'',_nu=(a-2>=lo)&&(PREP[_pv]||NUM_DET[_pv]||(a-2>lo&&/^[A-ZÀ-Þ]/.test(T[a-2])));   /* ⭐ 11/09/2026 : pas un prénom NU derrière une préposition (« de MARS »), un déterminant (« Les DENIS ») ou un autre nom propre (« Di ROSA ») — 3 rouges ont→a sur texte correct. Miroir Python. */
      if(!_co&&!_nu&&!(a-1===lo&&_pg[1]))return{idx:a-1,det:a-1,dtxt:'',htxt:T[a-1],elid:false,g:_pg[0],n:'s'};}
  ...
  function _sujetFlexion(T,i,tg){   /* (personne, nombre) du sujet de T[i] — miroir Python _sujet_flexion. On remonte jusqu'à la
    ...
    var j=i-1;
    while(j>=0){
      if(_SEG&&j+1<_SEG.bb.length&&_SEG.bb[j+1]&&j+1!==i)return null;
      var w=T[j],d=deacc(w.toLowerCase()),m=_ELIDED_PRON.exec(w.toLowerCase());
      if(m)d=deacc(m[1]);
      if(SUBJ_PRON[d])return [SUBJ_PRON[d][0],SUBJ_PRON[d][1],'pron'];
      if(_SUBJ_PRON_PL[d]){
        var k=j-1;
        while(k>=0){
          if(_SEG&&k+1<_SEG.bb.length&&_SEG.bb[k+1])break;
          var dk=deacc(T[k].toLowerCase());
          if(_ELIDED_PRON.exec(T[k].toLowerCase()))return null;
          if(SUBJ_PRON[dk]||_SUBJ_PRON_PL[dk]||dk==='qui')return null;
          if(/^(que|qu)$/.test(dk.replace(/['’]$/,''))||(tg&&k<tg.length&&tg[k]==='SCONJ'))break;   // la subordonnée COMMENCE ici
          if(tg&&k<tg.length&&(tg[k]==='NOUN'||tg[k]==='PROPN'||tg[k]==='VERB'||tg[k]==='AUX'))return null;
          if(CLITIC[dk]||(tg&&k<tg.length&&tg[k]==='ADV')){k--;continue;}
          return null;
        }
        return [_SUBJ_PRON_PL[d][0],_SUBJ_PRON_PL[d][1],'pron'];
      }
      if(d==='qui')return null;
      if(svReads(w).length)return null;
      if(tg&&j<tg.length&&(tg[j]==='NOUN'||tg[j]==='PROPN')){var _np=_npSubject(T,tg,i);if(!_np||(_np.n!=='s'&&_np.n!=='p'))return null;   // sujet NOMINAL → palier ORANGE (miroir Python)
        var _dd=deacc(String(_np.dtxt||'').toLowerCase());if((_dd==='des'||_dd==='du'||_dd==='au'||_dd==='aux')&&_np.idx>=2){var _av=_np.idx-2;if(_av>=0&&(svReads(T[_av]).length||(tg&&_av<tg.length&&(tg[_av]==='NOUN'||tg[_av]==='PROPN'||tg[_av]==='VERB'||tg[_av]==='AUX'))))return null;}   /* ⭐ 12/09/2026 : contracté des/du/au/aux derrière un verbe ou un nom = COMPLÉMENT, pas le sujet (miroir Python, garde absente ici) — « dégoûtés du système partent » ne donne plus « part » */
        return ['3',_np.n,'nom'];}
      if(CLITIC[d]||(tg&&j<tg.length&&tg[j]==='ADV')){j--;continue;}
      if(_ADJ_ANTE[d]||(tg&&j<tg.length&&tg[j]==='ADJ')){j--;continue;}   // « les petits chats manges » : l'adjectif antéposé se traverse
      return null;
    }
    return null;}
  ...
  function sujFlexVig(T,i){   /* « nous iriez » → irions · « tu irai » → iras. Le sujet donne personne et nombre, la forme écrite
    ...
    if(MODAL[dl]&&i+1<T.length&&COMMON_VERBS[deacc(T[i+1].toLowerCase())])return null;   // MODAL seulement devant un VRAI INFINITIF (« je vais manger ») — miroir EXACT du Python (VERB_LEX ≡ COMMON_VERBS). L'ancien test « le mot suivant n'a aucune lecture » était vrai de « au », « le », « des »… : « Nous allez AU parc » restait muet DANS LE PRODUIT alors que la référence le corrigeait
    if(rAuxMisspell(T,i))return null;   // ⭐ si la règle des auxiliaires parle (« nous somme » → sommes), celle-ci se tait : conjuguer *sommer* par-dessus donnait « sommons ». Renvoi explicite, indépendant de l'ordre du registre — miroir Python
    ...
    if(!_lecC.length){   // la forme n'est PAS une case exacte d'un paradigme → les gardes nom/adjectif s'appliquent (miroir Python)
      if(_looksPpl(w))return null;                                          // participe épithète : « Les randonneurs épuisés »
    ...
    if(_lecC.length)lec=_lecC;   // ⭐ 12/09/2026 : AVANT le filtre « que » (ordre Python) — réinjectée APRÈS, la lecture subjonctive (sommer sub:pre) revenait dans les cibles, case 2p absente → null : « vous sommes » restait muet côté produit
    ...
    for(k=0;k<lec.length;k++){   // PAS svAgrees : elle ignore le nombre en 1re/2e
      ...
      var _lfr=_lemFreq(lec[k][0]);   /* ⭐ 12/09/2026 : « tu sommes » passait pour ACCORDÉ par *sommer* (0,23/M) alors qu'être (433/M) demande « es » — un lemme RARE ne prouve pas l'accord (miroir Python) */
      if(typeof _lfr==='number'&&_lfr<1){var _autre=false;for(var _k2=0;_k2<lec.length;_k2++){var _q2=_lemFreq(lec[_k2][0]);if((_q2===null||_q2>=1)&&(lec[_k2][2]!==per||(lec[_k2][3]!==nb&&lec[_k2][3]!=='x')))_autre=true;}if(_autre)continue;}
      return null;}
    var que=false;
    for(k=Math.max(0,i-5);k<i;k++)if(/^(que|qu)$/.test(deacc(T[k].toLowerCase()).replace(/['’]$/,'')))que=true;
    var lec2=[];
    for(k=0;k<lec.length;k++)if((lec[k][1].indexOf('sub')===0)===que)lec2.push(lec[k]);
    if(lec2.length)lec=lec2;                                                // « que » présent → subjonctif ; absent → indicatif
    var cible=null,slot=per+nb,lemc=null,_cands=[];
    for(k=0;k<lec.length;k++){
      var f=_CONJ_FIX[lec[k][0]+'|'+lec[k][1]+'|'+slot]||((CONJ_C[lec[k][0]]||{})[lec[k][1]]||{})[slot];
      if(!f)return null;
      _cands.push([lec[k][0],f]);
    }
    var _formes={};for(k=0;k<_cands.length;k++)_formes[_cands[k][1]]=1;
    if(Object.keys(_formes).length>1){   /* ⭐ 12/09/2026 (« vous sommes » → êtes, cas de Rem) : deux lemmes, deux formes — le lemme RARE (< 1/M : sommer) s'efface devant le courant (être) ; deux lemmes courants (« vis » vivre/voir) → abstention comme avant. Miroir Python. */
      var _forts=[];for(k=0;k<_cands.length;k++){var _q=_lemFreq(_cands[k][0]);if(_q===null||_q>=1)_forts.push(_cands[k]);}
      var _ff={};for(k=0;k<_forts.length;k++)_ff[_forts[k][1]]=1;
      if(Object.keys(_ff).length!==1)return null;
      _cands=[_forts[0]];}
    lemc=_cands[0][0];cible=_cands[0][1];
    ...
    /* ⭐ LEMME RARE À UNE LETTRE D'UN AUXILIAIRE (10/09/2026, cas de Rem) : « nous somme » → sommons, « vous somme » → sommez —
       *sommer* est un verbe que personne n'emploie (0,23/M) et l'écrit est à une lettre de « sommes » (433/M) : la lecture
       « verbe rare » est la mauvaise hypothèse. Abstention ; rAuxMisspell parle s'il peut. ⚠️ On pèse le LEMME (SP.D2A →
       max des variantes, comme _lemfreq côté Python), PAS la forme cible : peser la forme taisait « nous fîtes » → fîmes
       (passé simple rare d'un verbe courant, à une lettre de fûmes). Lemme inconnu du speller → on ne change rien. */
    var _lf;if(lemc&&typeof SP!=='undefined'&&SP&&SP.ready&&SP.D2A&&SP.FREQ){var _va=SP.D2A[lemc];if(_va&&_va.length){_lf=0;for(var _z=0;_z<_va.length;_z++)if((SP.FREQ[_va[_z]]||0)>_lf)_lf=SP.FREQ[_va[_z]]||0;}}
    if(typeof _lf==='number'&&_lf<1){var _vx=_voisinAuxLong(dl);if(_vx){   /* ⭐ 12/09/2026 : plus de silence — l'écrit est à une lettre d'une forme longue d'ÊTRE/AVOIR (« vous somme » ↔ sommes) : c'est CET auxiliaire, conjugué pour le sujet → êtes. Miroir Python. */
  ...
  function rAdjEpithet(T,i){var _el=(i>=1&&_elidKind(T[i-1])==='det');
    ...
    var _hdG=_headText(T[i-1]).toLowerCase(),dn=deacc(_hdG),g=_GCOLL[_hdG]||GENDER_PURE[dn];   /* ⭐ 18/09/2026 (cas de Rem : « le marché provençale », muet) — « marché » (m) et « marche » (f) partagent la clé nue « marche » : GENDER_PURE ne connaît ni l'un ni l'autre, et la règle se taisait. La table des COLLISIONS D'ACCENT (_GCOLL, celle de rDetGenre) répare exactement cette perte. ⚠️ PAS la table accentuée brute (_GACC) : essayée et mesurée sur 14 450 phrases correctes, elle donne « f » aux noms épicènes et fabriquait 8 ROUGES faux (« un peintre italien » → italienne) — que la référence Python avait, et n'a plus. */
  ...
  var _VIG_FAM={'genre déterminant':1,'leur/leurs':1,'accord participe':1,'ce/se':1,'élision inversée':1};   // ⭐ 13/09/2026 : élision inversée rouge seulement dans ses sous-cas sûrs (_deselide)
...
function _levB(a,b,max){if(Math.abs(a.length-b.length)>max)return max+1;var pr=[],cu=[],i,j;for(j=0;j<=b.length;j++)pr[j]=j;for(i=1;i<=a.length;i++){cu[0]=i;var bst=i;for(j=1;j<=b.length;j++){var v=Math.min(pr[j]+1,cu[j-1]+1,pr[j-1]+(a.charAt(i-1)===b.charAt(j-1)?0:1));cu[j]=v;if(v<bst)bst=v;}if(bst>max)return max+1;for(j=0;j<=b.length;j++)pr[j]=cu[j];}return pr[b.length];}   // distance d'édition BORNÉE
  var _VOY_RENDUE='aeiouy',_VOY_FREQ=50;   /* plancher MESURÉ : 0 → 14/7/7 · 50 → 14/2/2 · 200 perd fort et nord */
  function _voyelleMangee(d){var best=null,bf=0,i,k,q,L,f;
    for(i=0;i<=d.length;i++)for(k=0;k<_VOY_RENDUE.length;k++){
  ...
  function spellTokenCore(tok,atStart,T,idx){
    ...
    var d=deaccS(low);
    if(!/[aeiouy]/.test(d)){/* ⭐ LA VOYELLE MANGÉE (09/09/2026, miroir Python _voyelle_mangee) : cette garde écartait le SIGLE (www, mph, bzh) ET la faute dys — « snt », « frt », « nrd », « dns », « pls ». Mesuré au moteur : 111 jetons sans voyelle lui échappaient, dont 84 FAUTES pour 27 vrais sigles. On rend UNE voyelle et rien d'autre (aucune consonne touchée, aucune substitution : « dns » → dans, jamais « des »), puis un plancher de fréquence — le mot d'un dys est courant, le voisin d'un sigle ne l'est pas. 14 justes, 2 fausses, 2 marques sur mot correct (chf→chef, abréviation), toutes en ORANGE. ⛔ La seule SOUS-SÉQUENCE a été RÉFUTÉE : kms→keums, pqr→piquer (15 justes / 53 fausses). */var _vm=_voyelleMangee(d);return _vm?['vigilance',_vm]:null;}
    ...
    var _dw=null,_df=0;for(var _q=1;_q<low.length-1;_q++){if('bcdfglmnprst'.indexOf(deaccS(low[_q]))<0)continue;var _cd=low.slice(0,_q+1)+low[_q]+low.slice(_q+1);var _f=SP.WORDS.has(_cd)?(SP.FREQ[_cd]||0):0;if(_f>=3&&_f>_df){_dw=_cd;_df=_f;}}/* ⭐ GARDE DE L'ACCENT (08/09/2026 circonflexe, 11/09 aigu/grave non verbal ; miroir de _accent_rival) : cette route rendait son verdict AVANT les candidats, donc sans avoir vu la restauration d'accent (priorité 2 du speller). Le circonflexe français est la TRACE d'une consonne DISPARUE (cône < konos, bâton < baston, hôpital < hospital) : circonflexe et consonne doublée sont deux reconstructions RIVALES de la même consonne — doubler ne peut pas affirmer seul. Garde limitée à une saisie SANS accent (sinon le rival serait une DÉ-accentuation : alé→ale, caré→care — et là doubler a raison). Mesuré : 308 jetons sur cette route, 2 changent (cone→cône, batons→bâtons, gold ×2 et ×3), 0 correction perdue, 0 tir sur 354 647 jetons d'UD ; placebo sur l'accent aigu = 5 changements dont 4 faux au gold. */if(_dw){var _cr=false;if(deaccS(low)===low){var _dL=SP.D2A[low]||[];for(var _dk=0;_dk<_dL.length;_dk++){var _drw=_dL[_dk];if(_drw!==low&&deaccS(_drw)===low&&(SP.FREQ[_drw]||0)>=1&&(/[âêîôûÂÊÎÔÛ]/.test(_drw)||((SP.POS[_drw]||'').indexOf('V')<0&&(SP.FREQ[_drw]||0)>=_df))){_cr=true;break;}/* ⭐ 11/09/2026 : l'AIGU/GRAVE compte aussi, si le rival n'est PAS une forme VERBALE (POS sans V) — désert/dessert sont deux noms, la fréquence tranche ; jette/jeté est une alternance verbale que la GRAMMAIRE tranche (« il a jete » → jeté), pas le speller. ET au moins aussi fréquent que le mot doublé : « guere » → guère (9,7/M, adverbe) cédait alors que le dys voulait guerre (184/M, gold ×2) — la batterie l'a vu. Recensé dans toute la table : 4 paires (guere, tele, desert, teles) ; avec la fréquence, désert et télé gagnent, guerre et telles restent au doublement. Miroir _accent_rival. */}}if(!_cr)return['flag',_dw];}   // DOUBLE-CONSONNE simplifiée (laisé→laissé, pome→pomme, carote→carotte) = faute dys fréquente → restauration PRIORITAIRE. FP=0 UD 2500. Miroir Python/app.
...
/* OMISSION — NE PAS RACCOURCIR CE QUE L'UTILISATEUR A DÉJÀ RACCOURCI. Miroir exact de l'app.
    ...
    /* ⭐ PRÉSENT APRÈS PRONOM SUJET (13/09/2026, colonne accents : « il decide de » → le produit écrivait « décidé », le gold veut « décide » —
       ...
       et rendait « décidée ». Recensé (toutes sources : 6 cas, 6 présents) avant d'être posé. Miroir Python. */
    var _sbj=false;if(T&&/e$/.test(d)){var _y=idx-1;while(_y>=0){var _dy=deaccS(T[_y].toLowerCase()).replace(/'/g,'');if(SAUXAV[_dy]||SCOPULA[_dy])break;if(SSUBJP[_dy]||_dy==='qui'){_sbj=true;break;}
      if(_dy==='ne'||_dy==='n'||_dy==='se'||_dy==='s'||_dy==='me'||_dy==='m'||_dy==='te'||_dy==='t'||_dy==='le'||_dy==='la'||_dy==='les'||_dy==='l'||_dy==='lui'||_dy==='leur'||_dy==='y'||_dy==='en'){_y--;continue;}break;}}
    if(_sbj){var _pr=null,_ppl=false;for(i=0;i<keys.length;i++){var _ck=keys[i];if(deaccS(_ck)!==d)continue;if(/é$/.test(_ck))_ppl=true;else if(/e$/.test(_ck)&&cand[_ck][1]>=0.5&&(!_pr||cand[_ck][1]>cand[_pr][1]))_pr=_ck;}
      if(_pr&&_ppl&&_pr!==w1)return['flag',_pr];}   // les DEUX accentuations existent (présent -e / participe -é) et le tri a pris le participe → le présent (plancher 0,5 : « trébuche » 0,85/M)
...
function spellUnknown(tok,atStart,T,idx){
  ...
  var _EPI_ART_AU={au:'s',aux:'p'};   // (une garde « locution : au final, au total » a été essayée puis RETIRÉE : 0 différence sur 14 450 phrases UD et aucune phrase construite ne la sollicitait — le nom doit déjà avoir un genre PUR)
  function genreAdjVig(T,i,tg){var _el=(i>=1&&_elidKind(T[i-1])==='det');
    ...
    var _a2=(i>=2)?deacc(T[i-2].toLowerCase()):'',num=_el?'s':(_EPI_ART[_a2]||_EPI_ART_AU[_a2]||null);   /* ⭐ 18/09/2026 — « au / aux » : l'article CONTRACTÉ manquait à la table (« nous allons au marché provençale », « au niveau mondiale » ×2 dans le corpus dys). Pour cette règle ORANGE seulement : la rouge garde sa table mesurée. */
    if(!num)return null;
  ...
  function vigAt(T,i,st,text){var _tg=st.tg;   /* ⭐ 12/09/2026 — la CHAÎNE ORANGE de spellText, extraite TELLE QUELLE (ordre compris) : spellText la consulte token par token (sortie identique), diagnoseAll la consulte à UN index sur des tokens où l'orthographe est corrigée (bout de chaîne orthographe → orange). st.tg = tags du tagger, calculés une fois par liste de tokens ; text = null hors spellText (le juge B2 de l'app ancre le TEXTE, il ne se consulte pas sur des tokens corrigés). */
    ...
    {if(_tg===null)_tg=st.tg=posTags(T)||[];var pe=participeEtreVig(T,_tg,i);if(pe){return {i:i,word:T[i],sugg:pe,name:'accord participe à vérifier',tier:'vigilance'};}}
    {if(_tg===null)_tg=st.tg=posTags(T)||[];if(_tg[i]==='VERB'||_tg[i]==='AUX'){var sva=rAccordSVnoun(T,i,true);   // ACCORD SUJET-VERBE mid-phrase (rouge = sujet en tête FP=0 ; orange = le reste). DOCTRINE : doute → orange, jamais silence. Fusion grammaire-prioritaire : le rouge gagne au même token.
      if(sva&&sva.toLowerCase()!==T[i].toLowerCase()&&!svTaisCarte(T,i)){return {i:i,word:T[i],sugg:ckeepcase(T[i],sva),name:'accord sujet-verbe à vérifier',tier:'vigilance'};}}}
    ...
    {var gv2=gerondifVig(T,i);if(gv2&&gv2.toLowerCase()!==T[i].toLowerCase()){return {i:i,word:T[i],sugg:gv2,name:'participe présent après « en » à vérifier',tier:'vigilance'};}}
    {var spv=sestPpVig(T,i);if(spv){return {i:i,word:T[i],sugg:spv,name:'accord participe à vérifier',tier:'vigilance'};}}   // « elle s'est marié » → mariée ? (pronominal = orange)
  ...
  function spellText(text,capital){if(SP.RARE&&!SP.PHONR&&typeof setTimeout==='function')(function _st(){if(!_phonRareStep(4000)||!_anaStep(4000))setTimeout(_st,0);})();   // ⭐ 13/09/2026 : index des mots rares bâti en tâche de fond dès la première analyse
    ...
    if(!pushed){var _vg=vigAt(T,i,_vst,text);if(_vg){out.push(_vg);pushed=true;}}}   // ⭐ 12/09/2026 : la chaîne orange vit dans vigAt (consultable à UN index sur des tokens corrigés — bout de chaîne orthographe → orange)
...
var byTok={};gf.forEach(function(f){byTok[f.i]=f;});sf.forEach(function(f){if(byTok[f.i]==null)byTok[f.i]=f;
    ...
    /* ⭐ BOUT DE CHAÎNE orthographe → orange (12/09/2026) — les règles orange (vigAt) lisent le mot BRUT : sur « la France a réusie a
       ...
       0 marque née, 1 suggestion changée sur un faux positif du speller. SANS PLAFOND (un plafond de 12 perdait « réusie », 60e correction
       de sa copie) : le tagger n'est relancé que sur ±30 mots autour du mot corrigé (étiquettes du texte brut ailleurs) — re-étiqueter
       tout le texte à chaque pas coûtait +39 % sur 5 000 mots. Miroir Python bout_de_chaine_orange. */
    var _segSp=_segInfo(text),_tgR=null;
    flags.forEach(function(f){if(f.name!=='orthographe'||(f.tier!=='flag'&&f.tier!=='vigilance')||f.chaine||typeof f.i!=='number'||(f.span&&f.span>1)||typeof f.sugg!=='string'||!/^[A-Za-zÀ-ÿœŒ']+$/.test(f.sugg)||f.sugg.toLowerCase()===String(f.word).toLowerCase()||f.i>=_T.length)return;
      var _Tv=_T.slice();_Tv[f.i]=f.sugg;var _o=null;
      try{if(_tgR===null)_tgR=posTags(_T)||[];var _a=Math.max(0,f.i-30),_b=Math.min(_Tv.length,f.i+31),_tw=posTags(_Tv.slice(_a,_b))||[],_tv=_tgR.slice();for(var _q=_a;_q<_b;_q++)_tv[_q]=_tw[_q-_a];
