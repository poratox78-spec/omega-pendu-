...
(function (global) {
  ...
  function rAccordSVnoun(T,i,vig){var lw=T[i].toLowerCase();if(lw.indexOf("'")>=0||lw==='à')return null;
    ...
    var tg=posTags(T);if(!tg||i>=tg.length)return null;
    if(tg[i]!=='VERB'&&tg[i]!=='AUX'){var _di=deacc(T[i].toLowerCase());if(GENDER_MAP[_di]||ADJP[_di])return null;}   // filet homographe : l'émission HMM (~2 % des formes) rate « persiste »/« bloque »/« signale » (→NOM/ADJ) ; nom/adj CONNU (gêne/reste/jeune) → abstention, les gardes structure ci-dessous tranchent. Miroir Python rule_accord_sv_noun.
