...
| 13 | **Prochaine mise à jour** : `version` **strictement supérieure** à celle publiée | ✅ montée à **0.6.6** le 09/09/2026 — la 0.6.4 a été téléversée le jour même et la **0.6.5 a été annoncée prête** à Rem : un numéro remis est brûlé, la suivante devait donc être > 0.6.5 |
| 14 | Le bloc **« 🔢 Aide au nombre »** est décrit dans la fiche | ✅ paragraphe du §3 collé à l'envoi du 28/08/2026 |
| 15 | **Paquet à téléverser** : `omega-correcteur-dys-store.zip` (manifest à la racine, v0.6.6) | ✅ régénéré le 09/09/2026 par `python3 extension/build_zip.py --store` sur `main` — ⬜ téléversement par Rem quand il décide d'une nouvelle revue (la 0.6.3 est encore en revue ; la prochaine devra être > 0.6.3) |
