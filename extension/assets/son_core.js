...
var OmegaDysSonCore = (function () {
  ...
  // préfixes ÉLIDÉS (l'école, j'ai, c'est, qu'il, jusqu'à…) — MÊME table que build_son_layer.py
  var ELIDE = {
    l:[['l','l']], j:[['j','Z']], d:[['d','d']], n:[['n','n']], c:[['c','s']], s:[['s','s']],
    m:[['m','m']], t:[['t','t']], qu:[['qu','k']],
    jusqu:[['j','Z'],['u','y'],['s','s'],['qu','k']], lorsqu:[['l','l'],['o','O'],['r','R'],['s','s'],['qu','k']],
    puisqu:[['p','p'],['ui','8i'],['s','s'],['qu','k']], quoiqu:[['qu','k'],['oi','wa'],['qu','k']]
  };
  function recase(word, pairs) {                         // graphèmes (minuscules) → tranches du mot ORIGINAL ; null si non couvrant
    var lw = word.toLowerCase(), cat = '', i;
    for (i = 0; i < pairs.length; i++) cat += pairs[i][0];
    if (cat !== lw) return null;
    var segs = [], pos = 0;
  ...
  function wordSegments(word, g2p) {
    var parts = word.split(/(['’])/);                    // élision : « l'école » → l + ' + école, chaque partie à sa route
    if (parts.length > 1) {
      var out = [], p;
      for (p = 0; p < parts.length; p++) {
        var part = parts[p];
        if (!part) continue;
        if (part === "'" || part === '’') { out.push({g: part, ph: '', cls: 'n'}); continue; }
        var pre = (p + 1 < parts.length) ? ELIDE[part.toLowerCase()] : null;
        var segs = pre ? recase(part, pre) : null;
        out = out.concat(segs || wordSegments(part, g2p));
      }
      return out;
    }
    ...
    // couvre pas le mot (œ, caractère inconnu…), UN segment neutre → le texte reste intact.
    return recase(word, pairs) || [{g: word, ph: '', cls: 'n'}];
  }
