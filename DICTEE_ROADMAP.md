...
- **Gold Acadomia** (`dictee/homophones_probe.py`, exercice corrigé d'un prof) : **1 faute /8 attrapée, 0 FP**. → **Le correcteur est FP=0 mais à RAPPEL pauvre sur les homophones de manuel** : les règles ne firent qu'après un **sujet-PRONOM** (« le bébé **à** », « les poules **son** » = sujets-noms → ratés). **NB — contraste : la règle être/avoir USAGE, elle, est VALIDÉE** par les fiches le-stylo-de-vero (4/4 fautes en-cadre attrapées : « tu es peur »→as, « elle est 10 ans »→a, « il a allé »→est, « nous sommes raison »→avons ; 0 FP ; hors-cadre copule/possession correctement abstenu). C'est un **cas de réussite** — le gap est sur les homophones purs, pas sur être/avoir.

**Raffinage être/avoir (fiche « usage », mesuré 01/07).** `AVOIR_IDIOM` sûr = {faim, soif, sommeil, raison, tort, envie, besoin, peur} (être+X toujours faux). **NE PAS ajouter chaud/froid/mal** (« la soupe est chaude », « il est mal » valides → FP) **ni « temps »** (« il est temps de » valide) — vérifié, le correcteur ne les touche pas. Candidats sûrs mais marginaux : `l'air, l'habitude, l'intention, l'occasion` (« être l'air » jamais valide) ; éviter du mal / de la chance / le temps (1er token du/de/le trop générique). **Futur : il est / c'est** — « il est professeur » (nom nu) vs « c'est un professeur » (déterminant) → distinction déterminable par la présence d'un déterminant, mais FP-risquée (« il est un X » parfois acceptable) → à mesurer. « il y a » = pas une confusion.
...
**Plan :** (1) réécrire a/à + son/sont via substitution ; (2) quel/quelle (qu'il + genre) ; (3) c'est/s'est, près/prêt, met/mais, **ça/sa** (« cela »), + **familles à élision** dans/d'en, sans/s'en, ni/n'y, si/s'y, est/et (prép. vs pronom élidé — déterminable par structure) ; (4) **famille tout/tous/toute(s)/toux** (4 rôles — nom « ensemble » / pronom « cela·ils·elles »·effaçable / déterminant « le-les·chaque » + accord / adverbe « tout à fait », invariable sauf devant adj. fém. consonne·h-aspiré : complexe, à part) ; (5) lexicaux → vert ; (6) aligner `_isPpl` JS↔Python (21 FP de parité). Valider chaque ajout sur `homophones_probe.py` (rappel ↑, FP=0) + `fp_scale_probe`. Rien déployé sans décision (arbitrages FP↔rappel). Mémoire : `corrector-battery-findings`. *(Réfs PDF fournies — ça/sa, lexicaux, grammaticaux — non lisibles ici : pas de `pdftoppm` ; à recoller en texte si besoin.)*
...
- **Homophones grammaticaux tranchés par la GRAMMAIRE, en ROUGE** (classification manuelle de ~100 flags : **~82 % = vrais FP** au départ, sur les homophones — « a été »→à, conjonction « mais »→mes). ⚠️ **PAS** une démotion en vert (essai, puis rejeté : ce sont des fautes à rôle syntaxique, pas une simplification) → on utilise la double voie. Trous comblés : **couche segments** (frontière « qui, à »), **cadre auxiliaire** « a/ont + participe » (`_isPpl` : infinitif reconstruit ∈ lexique verbal → écarte noms en -é comme *actualité/portées*), **pronoms collés** (« qu'ils »), **inversion** (`hy` : « avait-on »), `son/sont` réécrite sur ce qui suit + `PLURAL_DET`, gardes début-de-phrase (« Mais ») + nom propre (« et Bob »). é/er durci (`NOUN_E`, **50→22**), accord-SV durci (aux+participe, **15→10**). `_isPpl`/`PLURAL_DET` (Python+JS), 3 moteurs, parité OK.

**Résultat : ROUGE 4,70 % → 1,73 % (Py et JS), homophones EN ROUGE** (tranchés par la grammaire), dont une part de vraies fautes (« la mode **à** largement contribué »→a). Parité app⊆Py & ext⊆Py OK, `recall_probe --check` OK. **Verrouillé en CI** par `dictee/fp_scale_probe.py` (correcteur sur 2500 phrases correctes UD, plafond 3 % — saute sur une régression FP type 4,70 %). Voir mémoire `corrector-fp0-at-scale`. *(NB : l'essai intermédiaire de « démotion en vert » des homophones a été REJETÉ — voir ÉTAT suivant.)*
