...
- 🔧 **2 leviers mesurés (`dictee/gender_levers_ud.py`) — à câbler (3 moteurs, parité)** :
  - **Déterminant — recall +6 pts, FP-safe** : relâcher la garde verbe-homographe (`P(VERB)<ε`) en gardant `P(NOUN)≥τ` — un mot après un déterminant à fort P(NOM) est un NOM même s'il est verbe-homographe. Mesuré UD : recall **66,8 → 72,7 %**, FP **0,09 → 0,10/1000** (+2 FP). *Gain propre, 1 ligne par moteur.*
  - **Adjectif — activable comme SOUS-ENSEMBLE FP-safe** : portée serrée (nom-tête immédiat pur + genre connu + **déterminant qui corrobore le genre**) → FP **3,37 → 0,19/1000** (÷18). Précis sur son périmètre, mais **étroit** (≈ 494 cas / 355 k → couverture faible). Active les cas faciles (« une voiture bleu→bleue ») ; le large reste = parsing. *Décision : câbler ce sous-ensemble ou non.*
