...
- **FALSIFIÉ — NE PAS REFAIRE** : (a) M3_d ne désambiguïse pas les homophones (aucune entrée sens/contexte) — `dictee/EXP_M3D_FALSIFIE.md` ; (b) **pendu de phrases** comme levier winrate : le partage de lettres fuit les fins → la valeur est dans la *déclaration*, mesurée marginale sur le moteur ≥7 — `evo/PHRASE_HANGMAN_PROBE.md`. *Nuance* : le **readout bPC** de M3_d (cLetterScore) est « peut-être utile » (+3,4, cheat-free) — `AUDIT_OMEGA.md` §3 ; (c) **C cognitif OOV** (`AUDIT_OMEGA §1.8-1.11`) : la cognition seule fait ~11 % OOV ; le levier est l'**agrégation** = n-gram (§1.7) branché en **voie sublexicale arbitrée OS** (§1.9, bascule auto par fiabilité, in-lex ~97 %/OOV ~60 %) + **gap-aware** (§1.10, plus proche voisin révélé à distance 1..4, **+2 pts OOV** = 1er gain cognitif réel). **FALSIFIÉS — ne pas refaire** : lissage par substrat `letterVecsSDIM`, pooling position-relative, et le **C *léger* appris** (maxent/GATE/POE, `evo/learned_c_probe.js`) — tous < gap-aware (le plus proche voisin domine, les lettres révélées sont corrélées). Au-delà = attention/RNN lourd-opaque pour ~3-5 pts → **déféré à arbitrage de Rem** (c'est là qu'IndexedDB des poids deviendrait justifié).
