...
- **COGNITION Phono→Ortho** (`dictee/p2g.py` + `build_p2g.py` → `p2g_table.json` ; `dictee/P2G.md`) : **l'inverse** du décomposeur (son→écriture, *le* point dur dys). Jointe **§3** (pas argmax) : beam-search marginalisant la segmentation latente, **croisant** émission apprise (route SON, via `decompose.aligned_units`) × prior orthographique bigramme (route ORTHO). Mesuré held-out : **top-1 14,4 % · top-5 47 % · top-10 60 %** (le croisement ortho vaut +2,2 ; plafond bridé par 2,3 homophones/mot). Compressibilité de la base mesurée (`compress_probe.py`) : 7,9× gzip, **17× factorisé**.
...
- `python3 dictee/build_p2g.py` puis `python3 dictee/p2g.py "wazo" | --mot oiseau | --measure | --demo` → cognition **phono→ortho** (jointe §3, croisement son×ortho). `python3 dictee/compress_probe.py` → compressibilité de la base.
