...
- **CORRECTEUR DYS** (`dictee/correcteur_probe.py` + panneau app « 🩹 Correcteur ») : détecte **et corrige** **sans corrigé** (1) les homophones grammaticaux (a/à, son/sont, on/ont, leur/leurs, -é/-er, peu/peux/peut, ce/se, et/est) ET (2) l'**accord SUJET-VERBE** à sujet **pronom** (« Je doit »→dois, « On ont »→a, « il sont »→est, « Tu chante »→chantes) via la table de conjugaison Lexique 4 `cgram_conj.json`. **0 faux positif** (30 phrases + 29 témoins + **98 phrases réelles GEC** + held-out) ; held-out accord SV **6/6 sur vocabulaire neuf**. UI semi-directe : clic = applique la correction + **stade**. **Parité app↔Python** en CI (`parity_corr.js`). Garde-fou anti-bruit Lexique : 1re/2e pers.=personne seule, infinitif écarté, nombre déduit de la morphologie, suggestion cross-validée. Détail : `dictee/CORRECTEUR.md`.
