...
TRANSLATIONS = [
    ...
    ('title="RÉAPPLIQUE la configuration optimale CHEAT-FREE (preset rapport §8.3). Depuis le 24/09/2026 elle est déjà active au chargement : ce bouton sert à y REVENIR après avoir touché aux réglages. Il remet TOUS les interrupteurs à leur valeur mesurée, pas seulement ceux qu on a changés.">⚙️ Config optimale</button>',
     'title="RE-APPLIES the optimal CHEAT-FREE configuration (report preset §8.3). Since 24/09/2026 it is already active on load: this button is how you COME BACK to it after touching the settings. It resets EVERY switch to its measured value, not only the ones you changed.">⚙️ Optimal config</button>'),
    ...
    # ⭐ 24/09/2026 — le rangement des contrôles et le menu des réglages (demande de Rem).
    # Sans ces entrées, la page anglaise afficherait « Jouer » et « Réglages du moteur ».
    ('<span class="ctl-titre">Jouer</span>', '<span class="ctl-titre">Play</span>'),
    ('<span class="ctl-titre">Moteur</span>', '<span class="ctl-titre">Engine</span>'),
    ('<span class="ctl-titre">Mesurer</span>', '<span class="ctl-titre">Measure</span>'),
    ('<span class="ctl-note">bancs, entraînement θ, balayages, exports — pour la recherche</span>',
     '<span class="ctl-note">benches, θ training, sweeps, exports — for research</span>'),
    ('<span class="ctl-titre">Réglages du moteur</span>', '<span class="ctl-titre">Engine settings</span>'),
    ('</b> actifs sur <b id="toggles-total">—</b> — cliquer pour ouvrir',
     '</b> active out of <b id="toggles-total">—</b> — click to open'),
    ('aria-label="Filtrer les réglages"', 'aria-label="Filter the settings"'),
    ("ui_filtreToggles('tous')\">Tous <b>", "ui_filtreToggles('tous')\">All <b>"),
    ("ui_filtreToggles('cfg-optim')\">Config optimale <b>", "ui_filtreToggles('cfg-optim')\">Optimal config <b>"),
