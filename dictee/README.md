...
- **Décomposeur « à la Lexique 4 »** : `decompose.py` (+ `DECOMPOSE.md`) — lit/apprend des mots et les décompose en **SON et ORTHO** (double voie). Route sublexicale = `g2p()` de l'app extrait par `build_g2p_tables.py` → `g2p_tables.json` ; route lexicale = `phono_homophones.json` + `cgram_*`. Apprend = `learned_lex.json` (FP=0). Mesuré : sublexical 46 % exact / 88 % phonémique (seed 42, n=3000). Lancer : `python3 dictee/decompose.py --demo`.
