...
- **Red (FP=0)**: **subject-verb agreement**, gender, plural, é/er, leur/leurs, **grammatical homophones** (a/à, on/ont, son/sont, mais/mes, et/est, ce/se, peu — resolved by **syntax**: subject, agreement, auxiliary frame "a/ont + participle", glued pronouns, clause boundaries), **être↔avoir usage** ("il est faim"→"il a faim", "il a allé"→"il est allé"), **misspelled auxiliary** ("vous ete"→"êtes"), **sentence-start capital**. **Green (vigilance)**: confusables + **run-on** (missing punctuation). *Homophone FP=0 is held by grammar, not by a demotion to green — verified by a scale audit (UD corpus, 14,450 sentences: red flags 4.70%→1.73%, several of which are real errors caught; CI guard `fp_scale_probe`).*
