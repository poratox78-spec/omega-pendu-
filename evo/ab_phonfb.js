// OMEGA — A/B (a) : le miroir PHON descendant (M_PHON_FEEDBACK : M5_phon_m..M1_phon_m, dont M4_phon_m §1.4.3)
// se traduit-il en WINRATE ? Le signal +0,028 spécifique au mot (§1.4.3) fait-il gagner, ou est-il inerte comme M1_m ?
//
// A/B : config cognition, voie phon RÉELLEMENT active (init corrigé §1.4.3/§1.4.4), M_PHON_FEEDBACK ON vs OFF.
// In-lexique K=1, warmup 200 / test 100, graines appariées.
// Usage : node evo/ab_phonfb.js [warmupN] [testN] [seeds]   (défauts 200 / 100 / 8 graines)
'use strict';
const { loadEngine } = require('./fitness_harness.js');

// Cognition (declares OFF pour isoler la voie phon descendante, non masquée par NEO). Voie phon ON.
const CFG_COG = [
  ...
  'M_VOIE_PHON_ENABLED=true','M_OS_V07_ENABLED=true','M4_PHON_USE_P_ENABLED=true','M_SUBSTRAT_ORTHO_PURE_ENABLED=true',
...
(async () => {
  ...
  // ORDRE CORRIGÉ (§1.4.3) : config AVANT init pour bâtir la voie phon ; garde anti-inerte.
  function runCond(seed, sets, phonfb){
    ev(CFG_COG);
    ...
    ev(CFG_COG);
    ev(`M_PHON_FEEDBACK_ENABLED=${!!phonfb};`);
    if(ev("!(typeof M1_phon!=='undefined'&&M1_phon&&M1_phon.output)")) throw new Error('voie phon non bâtie (init §1.4.3)');
    if(ev("!!M_PHON_FEEDBACK_ENABLED")!==!!phonfb) throw new Error('M_PHON_FEEDBACK non posé');
  ...
  console.log(`\n=== A/B (a) miroir PHON descendant (M_PHON_FEEDBACK) — voie phon ACTIVE · in-lex K=1 · warmup ${warmupN}/test ${testN} · ${seeds.length} graines ===\n`);
  const on=[],off=[];
  for(const seed of seeds){
    const sets=pickSets(seed);
    const wOn=runCond(seed,sets,true), wOff=runCond(seed,sets,false);
    on.push(wOn);off.push(wOff);
    process.stderr.write(`  seed ${seed} : FB ON ${wOn.toFixed(1)}%  OFF ${wOff.toFixed(1)}%  Δ ${(wOn-wOff>=0?'+':'')}${(wOn-wOff).toFixed(1)}\n`);
  }
  const mean=a=>a.reduce((x,y)=>x+y,0)/a.length;
  const per=on.map((v,i)=>v-off[i]);
  console.log('  condition            '+seeds.map(s=>pad(s,7)).join('')+'  moyenne');
  console.log('  M_PHON_FEEDBACK ON   '+on.map(v=>pad(v.toFixed(0)+'%',7)).join('')+'  '+mean(on).toFixed(1)+'%');
  console.log('  M_PHON_FEEDBACK OFF  '+off.map(v=>pad(v.toFixed(0)+'%',7)).join('')+'  '+mean(off).toFixed(1)+'%');
  ...
  console.log(`  → ON>OFF ${w} · nul ${t} · ON<OFF ${l}\n  Lecture : Δ>0 net ⇒ le signal phon descendant (+0,028) fait gagner. Δ≈0 ⇒ existe mais inerte (comme M1_m).`);
