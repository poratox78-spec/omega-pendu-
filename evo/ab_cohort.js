...
(async () => {
  ...
  function runCond(seed, sets, { cohort, jointe, xmodal, dual, cfreq }) {
    ...
    ev(`M_NEO_COHORT_FREQ=${!!cfreq};`);            // fréquence CROISÉE dans la jointe (poids cohorte sur Pcoh(φ|p)), pas additionnée
  ...
  const conds = oov
    ...
    : (mode === 'freqx')
    ? [ { key: 'cohorte-jointe (sans CW)    ', cohort: true,  jointe: true,  cfreq: false },
        { key: 'cohorte-jointe + FREQ croisée', cohort: true, jointe: true,  cfreq: true  } ]
