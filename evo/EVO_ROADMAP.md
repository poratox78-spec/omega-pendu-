...
- ✅ **Volet SENS (06/2026) — avant-goût de sémantique, MESURÉ, et son plafond (`evo_o2_homophone_context.js`).** « Le sens = la compagnie qu'un mot fréquente » (Firth) : embeddings distributionnels, puis désambiguïsation d'homophones façon **Golding & Roth 1999** (Naïve Bayes **prior×contexte**, lissé, **held-out**, corpus **UD French-GSD** réel). Le contexte aide — **surtout les homophones GRAMMATICAUX** (son/sont 55→88, on/ont 57→87, ce/se 53→76, et/est 60→77, a/à 82→87 ; agrégé **68→81, +14 pts**) plus que sémantiques (mer/mère 65→88 ; le reste plafonné par le prior, +3). **MAIS précision ~77-88 % = LOIN du FP=0** → signal de **ré-ordonnancement** utile, **pas** un correcteur clé-en-main&nbsp;: ça **valide l'abstention** (« se taire plutôt qu'un FP »). **Clôture — y a-t-il un filet haute-confiance FP-safe ? (`evo_o2_sense_fpzero.js`)** NON&nbsp;: pour FP=0 il faut conf > 0,9999, et il ne reste que **1,6 % de recall** (à 99,9 % de conf, encore 8 flags à tort). **Aucun sous-ensemble FP-safe utilisable** — contraste mesuré avec le levier déterminant (lexical, propre) qui passe FP=0 *naturellement*. → le sens distributionnel est **définitivement recalé par le portier FP=0** (chiffré, pas supposé). Et c'est de la **collocation, PAS de la compréhension** (= LLM, autre projet, hors doctrine). *Contrôle littérature&nbsp;: un 1er jet (cosinus d'embeddings SEUL, sans prior, avec fuite) régressait sur a/à (−8) ; corrigé par prior×contexte (Golding&Roth) + lissage (Levy&Goldberg 2015) + held-out.* Corpus UD&nbsp;: hors-repo (`data_local/`, re-téléchargeable, cf. en-tête du script).
- ⚠️ **Sens × LanguageTool — le lexical passe FP=0 *sur UD* mais PAS en production (`evo_lt_confusion_fpzero.js`, 06/2026).** Cross-check : les **17 confusion sets lexicaux** que LanguageTool *active* (bon/bond, vin/vingt…, FA/10k≈0 via gros facteur) passés au **même portier FP=0** que les grammaticaux. Verdict : **7/7 mesurables passent FP=0** (0 recalé ; 7 trop rares dans UD), avec recall RÉEL — **père/paire 62 %, moi/mois 55 %, très/trait 31 %, prix/pris 18 %**. Cause : contextes **quasi disjoints** (« le **mois** de » vs « **moi** je ») → classifieur sûr ET ne touche jamais le correct. **Reproduit indépendamment la décision de LanguageTool** (il active le lexical, désactive ce/se, il/ils…). Donc : « sens recalé par FP=0 » = vrai **pour le grammatical**, **FAUX pour le lexical**. Caveats honnêtes : confiance NB **saturée** (% mous) ; **UD trop petit**. **Câblage TENTÉ puis ABANDONNÉ (doctrine FP=0).** père/paire + moi/mois câblées au réf Python (0 FP sur held-out UD + batterie), **mais une contre-épreuve réelle casse la garde** : « il porte une **paire** de lunettes » (correct) est flagué « père » (le groupe `/pER/` réel — pair/paire/perd/pers… — dépasse le modèle 2-voies ; UD trop étroit → seuil optimiste). **Règle retirée, monolithe NON touché.** → **Conclusion durcie : le sens distributionnel ne passe pas FP=0 en production, *lexical compris* ; seul le lexical-grammatical (genre/accord) y arrive.** (Mesures conservées : `lexical_confusion_wire_probe.js`.)
