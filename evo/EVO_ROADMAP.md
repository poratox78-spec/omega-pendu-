...
- ✅ **Sens × LanguageTool — le LEXICAL passe FP=0, le grammatical non (`evo_lt_confusion_fpzero.js`, 06/2026).** Cross-check : les **17 confusion sets lexicaux** que LanguageTool *active* (bon/bond, vin/vingt…, FA/10k≈0 via gros facteur) passés au **même portier FP=0** que les grammaticaux. Verdict : **7/7 mesurables passent FP=0** (0 recalé ; 7 trop rares dans UD), avec recall RÉEL — **père/paire 62 %, moi/mois 55 %, très/trait 31 %, prix/pris 18 %**. Cause : contextes **quasi disjoints** (« le **mois** de » vs « **moi** je ») → classifieur sûr ET ne touche jamais le correct. **Reproduit indépendamment la décision de LanguageTool** (il active le lexical, désactive ce/se, il/ils…). Donc : « sens recalé par FP=0 » = vrai **pour le grammatical**, **FAUX pour le lexical**. Caveats honnêtes : confiance NB **saturée** (% mous, classement solide) ; **UD trop petit** pour 7/14 paires. *Suite en cours : câbler père/paire · moi/mois · très/trait · prix/pris au correcteur (route « sens lexical @ haut seuil »), parité 3 moteurs, FP=0 prouvé Python d'abord.*
